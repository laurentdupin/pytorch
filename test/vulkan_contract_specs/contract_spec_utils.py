...
ADMISSION_DIAGNOSTICS_DECLARED_ROWS = (
    ...
    {
        "contract_name": "SafeViewReshapeContract",
        "family": "ReshapeAliasDenseBufferDirect",
        "tuple_id": "materialized_direct_buffer_reshape",
        "spec_file": "safe_view_reshape_alias_contract.json",
        "source_file": (
            "aten/src/ATen/native/vulkan/planning/"
            "ExecutionContractsSafeViewReshape.cpp"
        ),
...
def _sdpa_execution_policy_rowset(envelope, context):
    rowsets = envelope.get("sparse_rowsets", [])
    if len(rowsets) != 1:
        raise AssertionError(f"{context} must have one sparse rowset")
    rowset = rowsets[0]
    ...
    expected_fields = [
        ...
        "head_dim",
        "enable_gqa",
        "requires_materialized_math_path",
        "requires_score_pre_materialization",
        "requires_post_softmax_clone",
        "materialization_policy",
        "tuple_id",
    ]
...
def _sdpa_execution_policy_row_key_from_case(case):
    ...
    return (
        ...
        query_sequence_range[1],
        key_value_sequence_range[0],
        key_value_sequence_range[1],
        case["query_shape"][3],
        case["enable_gqa"],
    )
...
def _validate_sdpa_execution_policy_shape_envelope(file_name, spec, envelope):
    context = f"{file_name} SDPAExecutionPolicy ShapeEnvelope"
    _require_equal(
        spec["contract_name"],
        "SDPAExecutionPolicyContract",
        f"{context} contract",
    )
    _require_equal(spec["family"], "SparsePolicyRows", f"{context} family")
    _require_equal(envelope["bounds"], spec["bounds"], f"{context} bounds")
    ...
    for row in rows:
        family_counts[row["family"]] = family_counts.get(row["family"], 0) + 1
        row_key = _sdpa_execution_policy_row_key(row)
        row_keys.add(row_key)
        rows_by_key[row_key] = row
        lookup_keys.add(row_key)
        tuple_ids.add(row["tuple_id"])
    _require_equal(
        family_counts,
        {
    ...
    def require_qkv_case(case, case_context, allow_violations=()):
        ...
        _require_equal(
            case["key_shape"][1],
            case["value_shape"][1],
            f"{case_context} key/value heads",
        )
        _require_equal(
            case["key_shape"][2],
            case["value_shape"][2],
    ...
    for case in spec["positive_cases"]:
        case_context = f"{context} positive {case['name']}"
        require_qkv_case(case, case_context)
        row = rows_by_key[_sdpa_execution_policy_row_key_from_case(case)]
        _require_equal(
            case["expected_contract_tuple_id"],
            row["tuple_id"],
            f"{case_context} tuple id",
        )
        for key in (
            "requires_materialized_math_path",
            "requires_score_pre_materialization",
            ...
            _require_equal(
                case[f"expected_{key}"],
                row[key],
                f"{case_context} {key}",
            )
        _require_equal(
            case["expected_materialization_policy"],
    ...
    for case in spec["negative_cases"]:
        ...
        require_qkv_case(
            ...
            case_context,
            allow_violations=(
                "policy_rows.forbidden_cross_product",
                "policy_rows.diffusion_materialized_sequence",
                "policy_rows.diffusion_clone_sequence",
                "policy_rows.transformer_source_sequence",
                "scale_policy",
                "is_causal",
            ),
        )
        if case["violates"].startswith("policy_rows."):
            key = (
...
def _vision_self_attention_sdpa_rowset(envelope, context):
    rowsets = envelope.get("sparse_rowsets", [])
    if len(rowsets) != 1:
        raise AssertionError(f"{context} must have one sparse rowset")
    rowset = rowsets[0]
...
def _vision_self_attention_sdpa_row_key(row):
    return (
        row["family"],
        row["batch_heads"],
        row["query_sequence"],
...
def _vision_self_attention_sdpa_row_key_from_case(case):
    return (
        case["expected_contract_family"],
        case["query_shape"][0],
        case["query_shape"][1],
...
def _validate_vision_self_attention_sdpa_shape_envelope(file_name, spec, envelope):
    context = f"{file_name} VisionSelfAttentionSDPA ShapeEnvelope"
    _require_equal(
        spec["contract_name"],
    ...
    for key, expected in (
        ...
        ("is_causal", False),
        ("enable_gqa", False),
        ("scale_policy", "explicit_scale_1"),
    ):
        _require_equal(bounds[key], expected, f"{context} {key}")
    _require_equal(bounds["batch_heads"], [6, 12, 16], f"{context} batch_heads")
    _require_equal(bounds["query_sequence"], [151, 261], f"{context} query_sequence")
    _require_equal(
        bounds["key_value_sequence"],
        [151, 261],
        f"{context} key_value_sequence",
    )
    for key in ("requires_vulkan", "requires_buffer_storage", "requires_buffer_compute"):
        _require_equal(bounds[key], True, f"{context} {key}")

    rowset = _vision_self_attention_sdpa_rowset(envelope, context)
    rows = rowset["rows"]
    if len(rows) != 6:
        raise AssertionError(f"{context} expected 6 sparse rows")
    row_keys = {_vision_self_attention_sdpa_row_key(row) for row in rows}
    ...
    positive_keys = {
        _vision_self_attention_sdpa_row_key_from_case(case)
        for case in spec["positive_cases"]
    }
    if positive_keys != row_keys:
        missing = sorted(row_keys - positive_keys)
    ...
    def require_qkv_case(case, case_context, allow_violations=()):
        for shape_name in ("query_shape", "key_shape", "value_shape"):
            ...
            if len(shape) != 3:
                raise AssertionError(f"{case_context} {shape_name} rank mismatch")
            if (
                shape[0] != case["query_shape"][0]
                and case.get("violates") not in allow_violations
            ):
                raise AssertionError(f"{case_context} batch-head mismatch")
            if (
                shape[2] != case["query_shape"][2]
                and case.get("violates") not in allow_violations
            ):
        ...
        if (
            ...
            raise AssertionError(f"{case_context} enable_gqa mismatch")
        scale = case["scale"]
        if scale != 1.0 and case.get("violates") not in allow_violations:
            raise AssertionError(f"{case_context} scale mismatch")
    ...
    for case in spec["positive_cases"]:
        ...
        _require_equal(
            case["expected_route_label"],
            "SelectedVisionSelfAttentionSDPA",
            f"{case_context} route label",
        )
        _require_equal(case["expected_cpu_fallback"], False, f"{case_context} fallback")
    ...
    for case in spec["negative_cases"]:
        ...
        require_qkv_case(
            ...
            allow_violations=(
                ...
                "attention_rows.head_dim",
                "attention_rows.equal_sequence",
                "scale_policy",
                "is_causal",
                "dropout_zero",
            ),
...
def _validate_small_spatial_pointwise_conv_shape_envelope(
    ...
    rows = rowset["rows"]
    if len(rows) != 53:
        raise AssertionError(f"{context} expected 53 sparse rows")
    family_counts = {}
    row_keys = set()
    tuple_ids = set()
    lookup_keys = set()
    ...
    _require_equal(
        family_counts,
        {
            "DepthVisionProjection": 24,
            "OCRProjection": 13,
            "DiffusionProjection": 16,
        },
        f"{context} family counts",
    )
    _require_equal(len(tuple_ids), 53, f"{context} tuple ids")
    _require_equal(len(lookup_keys), 53, f"{context} lookup keys")
...
_SDPA_EXECUTION_POLICY_LEGAL_KEY_FIELDS = (
    "query_shape",
    "key_shape",
    "value_shape",
    "dtype",
...
_SDPA_EXECUTION_POLICY_ASSIGNMENT_COVERAGE_FIELDS = (
    "inputs.query.dtype",
    "inputs.query.rank",
    "inputs.query.dims.N",
    "inputs.query.dims.HQ",
...
_VISION_SELF_ATTENTION_SDPA_LEGAL_KEY_FIELDS = _TRANSFORMER_GQA_SDPA_LEGAL_KEY_FIELDS

_VISION_SELF_ATTENTION_SDPA_ADJACENT_NEGATIVE_KEY_FIELDS = (
    _TRANSFORMER_GQA_SDPA_ADJACENT_NEGATIVE_KEY_FIELDS
)

_VISION_SELF_ATTENTION_SDPA_ASSIGNMENT_COVERAGE_FIELDS = (
    "inputs.query.dtype",
    "inputs.query.rank",
    "inputs.query.dims.BH",
    "inputs.query.dims.TQ",
    "inputs.query.dims.D",
    "inputs.key.dtype",
    "inputs.key.rank",
...
SHAPE_ENVELOPE_ROLE_REGISTRY = {
    ...
    "sdpa_execution_policy_sparse_rows": {
        "validate": _validate_sdpa_execution_policy_shape_envelope,
        "assignment_cases": _generated_shape_envelope_assignment_cases,
        "legal_cases": _checked_in_shape_envelope_legal_cases,
        "adjacent_negative_cases": (
            _checked_in_shape_envelope_adjacent_negative_cases
        ),
        "legal_key_fields": _SDPA_EXECUTION_POLICY_LEGAL_KEY_FIELDS,
        "assignment_coverage_fields": (
            _SDPA_EXECUTION_POLICY_ASSIGNMENT_COVERAGE_FIELDS
        ),
        "adjacent_negative_key_fields": (
    ...
    "vision_self_attention_sdpa_sparse_attention_rows": {
        "validate": _validate_vision_self_attention_sdpa_shape_envelope,
        "assignment_cases": _generated_shape_envelope_assignment_cases,
        "legal_cases": _checked_in_shape_envelope_legal_cases,
        "adjacent_negative_cases": (
            _checked_in_shape_envelope_adjacent_negative_cases
        ),
        "legal_key_fields": _VISION_SELF_ATTENTION_SDPA_LEGAL_KEY_FIELDS,
        "assignment_coverage_fields": (
            _VISION_SELF_ATTENTION_SDPA_ASSIGNMENT_COVERAGE_FIELDS
        ),
        "adjacent_negative_key_fields": (
