...
class GraphModule(torch.nn.Module):
    def forward(self, L_x_: "DTensor(f32[4, 4], R)", L_mesh_ : torch.distributed.device_mesh.DeviceMesh):
        l_x_ = L_x_
        l_mesh_ = L_mesh_

        local: "f32[4, 4]" = l_x_.to_local();  l_x_ = None

        sin: "f32[4, 4]" = local.sin()
        cos: "f32[4, 4]" = local.cos();  local = None
        result: "f32[4, 4]" = sin + cos;  sin = cos = None

        from_local: "DTensor(f32[4, 4], R)" = torch.distributed.tensor._api.from_local(result, l_mesh_, [torch.distributed.tensor.placement_types.Replicate()]);  result = l_mesh_ = None
        return (from_local,)
""",  # noqa: B950
        )
