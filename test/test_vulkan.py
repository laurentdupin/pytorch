...
class TestVulkanGovernance(TestCase):
    ...
    def test_vulkan_shape_envelope_utility_cli(self):
        ...
        self.assertIn("validated 18 ShapeEnvelope v1 specs", result.stdout)
        self.assertIn(
            "batch_norm_inference_contract.json:"
            "batch_norm_inference_buffer_float_4d",
            result.stdout,
    ...
    def test_vulkan_shape_envelope_adjacent_negative_utility_cli(self):
        ...
        self.assertIn(
            "validated 18 ShapeEnvelope adjacent-negative generators",
            result.stdout,
        )
        self.assertIn("generated_cases=92", result.stdout)
    ...
    def test_vulkan_shape_envelope_legal_case_utility_cli(self):
        ...
        self.assertIn(
            "validated 18 ShapeEnvelope legal-case generators",
            result.stdout,
        )
        self.assertIn("generated_cases=98", result.stdout)
    ...
    def test_vulkan_shape_envelope_fuzz_assignment_utility_cli(self):
        ...
        self.assertIn(
            "validated 18 ShapeEnvelope fuzz assignment generators",
            result.stdout,
        )
        self.assertIn("legal_assignments=36", result.stdout)
        self.assertIn("adjacent_negative_assignments=91", result.stdout)
        self.assertIn(
            "batch_norm_inference_contract.json:legal=2:adjacent=3",
            result.stdout,
        )
        self.assertIn(
            "batch_norm_inference_materialized_contract.json:legal=2:adjacent=3",
            result.stdout,
        )
        self.assertIn(
        ...
        self.assertIn(
            "gqa_repeat_contract.json:legal=2:adjacent=2",
            result.stdout,
        )
        self.assertIn(
        ...
        self.assertIn(
            "no_overlap_conv_transpose2d_contract.json:legal=2:adjacent=5",
            result.stdout,
        )
        self.assertIn(
    ...
    def test_vulkan_shape_envelope_fuzz_assignment_coverage_cli(self):
        ...
        self.assertIn(
            "validated 18 ShapeEnvelope fuzz assignment coverage bridges",
            result.stdout,
        )
        self.assertIn("legal_assignments=36", result.stdout)
        self.assertIn("legal_paths=311", result.stdout)
        self.assertIn("adjacent_negative_axes=91", result.stdout)
        self.assertIn("runtime_legal_cases=98", result.stdout)
        self.assertIn("runtime_adjacent_negative_cases=92", result.stdout)
        self.assertIn(
        ...
        self.assertIn(
            "batch_norm_inference_materialized_contract.json:legal=2:"
            "status=covered:paths=21/21:adjacent_axes=3",
            result.stdout,
        )
        self.assertIn(
        ...
        self.assertIn(
            "gqa_repeat_contract.json:legal=2:status=covered:"
            "paths=10/10:adjacent_axes=2",
            result.stdout,
        )
        self.assertIn(
    ...
    def test_vulkan_shape_envelope_uses_role_registry_key_fields(self):
        ...
        self.assertEqual(
            set(registry),
            {
                ...
                "sdpa_execution_policy_sparse_rows",
                "elementwise_float_tensor_tensor_buffer_broadcast",
                "gqa_repeat_batch1_heads4_factor4_sequence100_to116_dim128",
    ...
    def test_vulkan_shape_envelope_sparse_rowset_canary(self):
        ...
        self.assertIn("validated 3 ShapeEnvelope sparse rowsets", result.stdout)
        self.assertIn("rows=56", result.stdout)
        self.assertIn("sparse_gap=193492", result.stdout)
        self.assertIn(
    ...
    def test_vulkan_generated_cpp_manifest_cli(self):
        ...
        self.assertIn(
            "validated 18 generated ShapeEnvelope C++ helper headers",
            result.stdout,
        )
    ...
    def test_vulkan_contract_coverage_census_cli(self):
        ...
        self.assertEqual(summary["specs"], 18)
        self.assertEqual(summary["generated_shape_envelope"], 18)
        ...
        self.assertEqual(summary["live_contract_without_json_spec"], 1)
        self.assertEqual(summary["exact_row_debt"], 1)
        ...
        self.assertEqual(
            spec_rows["diffusion_sdpa_contract.json"]["category"],
            "generated_shape_envelope",
        )
        self.assertEqual(
            spec_rows["sdpa_execution_policy_contract.json"]["category"],
            "generated_shape_envelope",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            spec_rows["gqa_repeat_contract.json"]["category"],
            "generated_shape_envelope",
        )
        self.assertEqual(
        ...
        self.assertFalse(
            spec_rows["diffusion_sdpa_contract.json"]["exact_row_debt"]
        )
        self.assertFalse(
            spec_rows["sdpa_execution_policy_contract.json"]["exact_row_debt"]
        )
        ...
        self.assertIn("TransformerGQASDPAContract", live_contracts)
        self.assertEqual(
            live_contracts["TransformerGQASDPAContract"]["category"],
        ...
        self.assertIn("validated contract coverage census specs=18", result.stdout)
        self.assertIn("generated_shape_envelope=18", result.stdout)
        self.assertIn("json_spec_without_shape_envelope=0", result.stdout)
        self.assertIn("live_contract_without_json_spec=1", result.stdout)
        self.assertIn("exact_row_debt=1", result.stdout)
    ...
    def test_vulkan_batch_norm_inference_materialized_contract_spec_shape(self):
        spec = _load_vulkan_contract_spec(
            "batch_norm_inference_materialized_contract.json"
        )
        self.assertEqual(spec["schema_version"], 1)
        self.assertEqual(spec["contract_name"], "BatchNormInferenceContract")
        self.assertEqual(spec["family"], "MaterializedBufferFloat4D")
        self.assertEqual(
            spec["tuple_id"],
        ...
        _require_contract_spec_fields(
            ...
            "BatchNormInferenceContract materialized metadata",
        )
        self.assertEqual(
            metadata["evidence_id"],
        ...
        _require_contract_spec_fields(
            ...
            "BatchNormInferenceContract materialized bounds",
        )
        self.assertEqual(bounds["input_dtype"], "float32")
        self.assertEqual(bounds["parameter_dtype"], "float32")
        self.assertEqual(bounds["input_rank"], 4)
        ...
        self.assertTrue(bounds["requires_vulkan"])
        self.assertTrue(bounds["requires_contiguous"])
        self.assertFalse(bounds["requires_buffer_storage"])
        self.assertTrue(bounds["requires_buffer_compute"])
        self.assertTrue(bounds["requires_materialization"])
        ...
        for section in ("positive_cases", "negative_cases"):
            ...
            for case in spec[section]:
                _require_contract_spec_fields(
                    case,
                    case_fields,
                    f"BatchNormInferenceContract materialized {section} case",
                )
                self.assertEqual(case["dtype"], bounds["input_dtype"])
                self.assertTrue(case["materialized_input"])
        ...
        for case in spec["positive_cases"]:
            _require_contract_spec_fields(
                ...
                (
                    ...
                    "expected_contract_materialization_policy",
                    "expected_cpu_fallback",
                ),
                "BatchNormInferenceContract materialized positive case",
            )
            self.assertEqual(case["expected_route_label"], spec["route_label"])
            self.assertEqual(case["expected_contract_family"], spec["family"])
            self.assertEqual(case["expected_contract_tuple_id"], spec["tuple_id"])
            self.assertEqual(
                case["expected_contract_materialization_policy"],
                metadata["materialization_policy"],
            )
            self.assertFalse(case["expected_cpu_fallback"])
        for case in spec["negative_cases"]:
    ...
    def test_vulkan_diffusion_sdpa_contract_spec_shape(self):
        spec = _load_vulkan_contract_spec("diffusion_sdpa_contract.json")
        self.assertEqual(spec["schema_version"], 1)
        self.assertEqual(spec["contract_name"], "DiffusionSDPAContract")
        self.assertEqual(spec["family"], "SparseAttentionRows")
        self.assertEqual(spec["tuple_id"], "diffusion_sdpa_sparse_attention_rows")
        self.assertEqual(spec["writer_op"], "aten::scaled_dot_product_attention")
        self.assertEqual(spec["route_label"], "SelectedDiffusionSDPA")
        ...
        _require_contract_spec_fields(
            ...
            "DiffusionSDPAContract metadata",
        )
        self.assertEqual(metadata["evidence_id"], "diffusion_sdpa_focused_tests")
        self.assertEqual(metadata["guard_id"], "diffusion_sdpa_adjacent_guards")
        self.assertEqual(metadata["fallback_policy"], "unsupported_shapes_do_not_match")
        self.assertEqual(
            metadata["materialization_policy"],
        ...
        _require_contract_spec_fields(
            ...
            "DiffusionSDPAContract bounds",
        )
        self.assertEqual(bounds["query_dtype"], "float32")
        self.assertEqual(bounds["key_dtype"], "float32")
        self.assertEqual(bounds["value_dtype"], "float32")
        self.assertEqual(bounds["query_rank"], 4)
        self.assertEqual(bounds["key_rank"], 4)
        self.assertEqual(bounds["value_rank"], 4)
        self.assertEqual(bounds["batch"], 1)
        self.assertEqual(bounds["heads"], [1, 5, 10, 20])
        self.assertEqual(bounds["query_sequence"], [12, 35, 126, 504, 640])
        self.assertEqual(
            bounds["key_value_sequence"],
            [2, 12, 35, 126, 504, 640],
        )
        self.assertEqual(bounds["head_dim"], [64, 512])
        self.assertFalse(bounds["has_attn_mask"])
        self.assertTrue(bounds["dropout_zero"])
        self.assertFalse(bounds["is_causal"])
        self.assertFalse(bounds["enable_gqa"])
        self.assertEqual(bounds["scale_policy"], "absent_or_head_dim_equivalent")
        self.assertTrue(bounds["requires_vulkan"])
        self.assertTrue(bounds["requires_buffer_storage"])

        rowset = spec["shape_envelope"]["sparse_rowsets"][0]
        self.assertEqual(rowset["name"], "attention_rows")
        self.assertEqual(len(rowset["rows"]), 11)
        family_counts = {}
        ...
        self.assertEqual(positive_keys, row_keys)
        ...
        for section in ("positive_cases", "negative_cases"):
            ...
            for case in spec[section]:
                _require_contract_spec_fields(
                    case,
                    case_fields,
                    f"DiffusionSDPAContract {section} case",
                )
                self.assertEqual(len(case["query_shape"]), 4)
                self.assertEqual(len(case["key_shape"]), 4)
                self.assertEqual(len(case["value_shape"]), 4)
        ...
        for case in spec["positive_cases"]:
            _require_contract_spec_fields(
                ...
                "DiffusionSDPAContract positive case",
            )
            self.assertFalse(case["expected_cpu_fallback"])
        for case in spec["negative_cases"]:
            _require_contract_spec_fields(
                ...
                "DiffusionSDPAContract negative case",
            )
            self.assertFalse(case["expected_native_route"])
            self.assertFalse(case["expected_cpu_fallback"])

    def test_vulkan_sdpa_execution_policy_contract_spec_shape(self):
        spec = _load_vulkan_contract_spec("sdpa_execution_policy_contract.json")
        self.assertEqual(spec["schema_version"], 1)
        self.assertEqual(spec["contract_name"], "SDPAExecutionPolicyContract")
        self.assertEqual(spec["family"], "SparsePolicyRows")
        self.assertEqual(
            spec["tuple_id"],
            "sdpa_execution_policy_sparse_policy_rows",
        )
        self.assertEqual(spec["writer_op"], "aten::scaled_dot_product_attention")
        self.assertEqual(spec["route_label"], "SelectedSDPAExecutionPolicy")
        ...
        _require_contract_spec_fields(
            ...
            "SDPAExecutionPolicyContract metadata",
        )
        self.assertEqual(metadata["evidence_id"], "sdpa_execution_policy_focused_tests")
        self.assertEqual(metadata["guard_id"], "sdpa_execution_policy_adjacent_guards")
        self.assertEqual(metadata["fallback_policy"], "unsupported_shapes_do_not_match")
        self.assertEqual(
            metadata["materialization_policy"],
            "per_policy_row_materialization",
        )
        ...
        positive_keys = {
            (
                ...
                case["policy_query_sequence_range"][1],
                case["policy_key_value_sequence_range"][0],
                case["policy_key_value_sequence_range"][1],
                case["query_shape"][3],
                case["enable_gqa"],
            )
            for case in spec["positive_cases"]
        ...
        for section in ("positive_cases", "negative_cases"):
            ...
            for case in spec[section]:
                _require_contract_spec_fields(
                    case,
                    case_fields,
                    f"SDPAExecutionPolicyContract {section} case",
                )
                self.assertEqual(len(case["query_shape"]), 4)
                self.assertEqual(len(case["key_shape"]), 4)
                self.assertEqual(len(case["value_shape"]), 4)
        ...
        for case in spec["positive_cases"]:
            _require_contract_spec_fields(
                ...
                (
                    ...
                    "expected_contract_family",
                    "expected_contract_tuple_id",
                    "expected_requires_materialized_math_path",
                    "expected_requires_score_pre_materialization",
                    "expected_requires_post_softmax_clone",
                    "expected_materialization_policy",
                    "expected_cpu_fallback",
                ),
                "SDPAExecutionPolicyContract positive case",
            )
            self.assertFalse(case["expected_cpu_fallback"])
        for case in spec["negative_cases"]:
            _require_contract_spec_fields(
                case,
                ("violates", "expected_native_route", "expected_cpu_fallback"),
                "SDPAExecutionPolicyContract negative case",
            )
            self.assertFalse(case["expected_native_route"])
            self.assertFalse(case["expected_cpu_fallback"])
    ...
    def test_vulkan_small_metadata_padded_conv2d_contract_spec_shape(self):
        ...
        self.assertEqual(spec["schema_version"], 1)
        self.assertEqual(spec["contract_name"], "SmallMetadataPaddedConv2DContract")
        self.assertEqual(spec["family"], "MaterializedBufferInput2x2")
        self.assertEqual(
            spec["tuple_id"],
        ...
        _require_contract_spec_fields(
            ...
            "SmallMetadataPaddedConv2DContract metadata",
        )
        self.assertEqual(
            metadata["evidence_id"],
        ...
        self.assertFalse(bounds["transposed"])
        self.assertFalse(bounds["options_quantized"])
        self.assertTrue(bounds["output_padding_is_zero"])
        self.assertTrue(bounds["requires_vulkan"])
        self.assertTrue(bounds["input_has_buffer_storage"])
        self.assertTrue(bounds["input_is_width_packed"])
        self.assertFalse(bounds["input_has_direct_buffer_layout"])
        self.assertTrue(bounds["input_supports_buffer_compute"])
        self.assertTrue(bounds["weight_defined"])

        layout = spec["shape_envelope"]["layout"]
        self.assertEqual(layout["input_layout"], "width_packed_small_metadata")
        self.assertEqual(layout["execution_storage"], "materialized_buffer")
        self.assertTrue(layout["requires_input_materialization"])
        ...
        for case in spec["positive_cases"]:
            _require_contract_spec_fields(
                ...
                "SmallMetadataPaddedConv2DContract positive case",
            )
            self.assertEqual(case["input_shape"], [1, 16, 721, 1281])
            self.assertEqual(case["out_channels"], 32)
            self.assertEqual(case["kernel_size"], [2, 2])
            self.assertEqual(case["expected_route_label"], spec["route_label"])
            self.assertEqual(
                case["expected_post_materialization_route_label"],
                "aten::convolution.buffer_float",
            )
            self.assertFalse(case["expected_cpu_fallback"])
            self.assertFalse(case["expected_sync_readback"])
        for case in spec["negative_cases"]:
            _require_contract_spec_fields(
                ...
                "SmallMetadataPaddedConv2DContract negative case",
            )
            self.assertFalse(case["expected_native_route"])
            self.assertEqual(
                case["expected_guard_route_label"],
    ...
    def test_vulkan_kv_cache_append_initial_contract_spec_shape(self):
        spec = _load_vulkan_contract_spec("kv_cache_append_initial_contract.json")
        self.assertEqual(spec["schema_version"], 1)
        self.assertEqual(spec["contract_name"], "KVCacheAppendContract")
        ...
        for case in spec["positive_cases"]:
            _require_contract_spec_fields(
                case,
                ("expected_route_label", "expected_cpu_fallback"),
                "KVCacheAppendContract InitialCache positive case",
            )
            self.assertEqual(case["expected_route_label"], spec["route_label"])
            self.assertFalse(case["expected_cpu_fallback"])
        for case in spec["negative_cases"]:
            _require_contract_spec_fields(
                ...
                "KVCacheAppendContract InitialCache negative case",
            )
            self.assertFalse(case["expected_native_route"])
            self.assertTrue(case["expected_cpu_fallback"])
            self.assertTrue(case["force_buffer_view"])
    ...
    def test_vulkan_no_overlap_conv_transpose2d_contract_spec_shape(self):
        ...
        _require_contract_spec_fields(
            ...
            (
                ...
                "input_supports_buffer_compute",
                "packed_defined",
                "execution_is_buffer_direct",
                "weight_has_buffer_storage",
                "bias_has_buffer_storage",
                "bias_is_float",
            ),
        ...
        self.assertEqual(bounds["weight_dtype"], "float32")
        self.assertEqual(bounds["input_rank"], 4)
        self.assertEqual(bounds["weight_rank"], 4)
        self.assertEqual(bounds["batch"], 1)
        self.assertEqual(bounds["input_channels"], {"min": 64})
        self.assertEqual(bounds["groups"], 1)
        self.assertEqual(bounds["kernel_h"], 2)
        self.assertEqual(bounds["kernel_w"], 2)
        self.assertEqual(bounds["stride_h"], 2)
        self.assertEqual(bounds["stride_w"], 2)
        self.assertEqual(bounds["padding_h"], 0)
        self.assertEqual(bounds["padding_w"], 0)
        self.assertEqual(bounds["dilation_h"], 1)
        self.assertEqual(bounds["dilation_w"], 1)
        self.assertTrue(bounds["transposed"])
        self.assertFalse(bounds["options_quantized"])
        self.assertFalse(bounds["packed_quantized"])
        self.assertTrue(bounds["output_padding_is_zero"])
        self.assertTrue(bounds["requires_vulkan"])
        self.assertTrue(bounds["input_has_buffer_storage"])
        self.assertTrue(bounds["input_supports_buffer_compute"])
        self.assertTrue(bounds["packed_defined"])
        self.assertTrue(bounds["execution_is_buffer_direct"])
        self.assertTrue(bounds["weight_has_buffer_storage"])
        self.assertTrue(bounds["bias_has_buffer_storage"])
        self.assertTrue(bounds["bias_is_float"])
        ...
        for section in ("positive_cases", "negative_cases"):
            ...
            for case in spec[section]:
                ...
                self.assertEqual(len(case["input_shape"]), bounds["input_rank"])
...
class TestVulkanEagerRuntime(VulkanDiagnosticLogMixin, TestCase):
    ...
    def _run_repo_python_subprocess(
        ...
        if extra_python_paths:
            pythonpath_entries.extend(extra_python_paths)
    ...
    def test_buffer_cast_matrix_core(self):
        ...
        with torch.inference_mode():
            ...
            bytes_to_float = bytes_vulkan.to(torch.float32)
            ...
            packet_like_view_to_float = (
                packet_like_vulkan.permute((0, 2, 3, 1))[0].to(torch.float32)
            )

            self.assertEqual(floats_to_int.dtype, torch.int32)
            self.assertEqual(ints_to_float.dtype, torch.float32)
            self.assertEqual(bytes_to_float.dtype, torch.float32)
            ...
            self._assert_outputs_close(
                ...
                rtol=1e-2)
            self._assert_outputs_close(
                packet_like.permute((0, 2, 3, 1))[0].to(torch.float32),
                packet_like_view_to_float.cpu(),
                atol=1e-5,
                rtol=1e-5)
    ...
    def test_transformers_mistral_logits_to_keep_on_vulkan(self):
        ...
        self._run_repo_python_subprocess(
            script,
            extra_python_paths=[self._benchmarks_python_path()],
            error_prefix="Transformers Mistral logits_to_keep path crashed on Vulkan.",
        )
    ...
    def test_safe_view_reshape_contract_generated_materialized_direct_buffer_spec(self):
        spec = _load_vulkan_contract_spec("safe_view_reshape_contract.json")
        op_hit_log_name = contract_spec_utils.contract_log_name(
            spec,
            {"name": "generated"},
            "op_hit_test.log",
            ...
            script = """
                ...
                def run_case(case, expect_native_route):
                    ...
                    try:
                        x = x_cpu.to("vulkan")
                        torch.ops.vulkan_prepack.reset_fallback_counters()
                        actual_vulkan = x.view(*case["output_shape"])
                        actual = actual_vulkan.cpu()
                    except RuntimeError:
    ...
    def test_kv_cache_append_contract_generated_initial_cache_spec(self):
        spec = _load_vulkan_contract_spec("kv_cache_append_initial_contract.json")
        op_hit_log_name = contract_spec_utils.contract_log_name(
            spec,
            {"name": "generated"},
        ...
        try:
            script = """
                ...
                spec_path = os.path.join(
                    ...
                    "test",
                    "vulkan_contract_specs",
                    "kv_cache_append_initial_contract.json",
                )
                with open(spec_path, encoding="utf-8") as handle:
                    spec = json.load(handle)
                ...
                def maybe_force_buffer_view(tensor, force):
                    vulkan = tensor.to("vulkan")
                    if not force or tensor.dim() != 4:
                        return vulkan
                    return vulkan.transpose(2, 3).transpose(2, 3)

                def make_empty(shape, dtype):
                ...
                def run_case(case, expect_native_route):
                    if os.path.exists(op_hit_log):
                        os.remove(op_hit_log)
                    torch.manual_seed(4000 + len(case["name"]))
                    dtype = dtype_for_case(case)
                    empty = make_empty(case["empty_shape"], dtype)
                    value = torch.randn(*case["value_shape"], dtype=dtype)
                    force_buffer_view = case.get("force_buffer_view", False)
                    empty_vulkan = maybe_force_buffer_view(empty, force_buffer_view)
                    value_vulkan = maybe_force_buffer_view(value, force_buffer_view)

                    torch.ops.vulkan_prepack.reset_fallback_counters()
                    expected = torch.cat((empty, value), dim=case["dim"])
                    actual = torch.cat(
                        (empty_vulkan, value_vulkan),
                        dim=case["dim"],
                    ).cpu()
                    torch.testing.assert_close(actual, expected, atol=1e-4, rtol=1e-4)
    ...
    def test_float_buffer_layer_norm_output_feeds_linear_without_runtime_label(self):
        materialize_log_name = "float_buffer_layer_norm_linear_unlabeled_materialize_test.log"
        op_hit_log_name = "float_buffer_layer_norm_linear_unlabeled_op_hit_test.log"
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        materialize_log_path = os.path.join(repo_root, materialize_log_name)
        op_hit_log_path = os.path.join(repo_root, op_hit_log_name)
        for path in (materialize_log_path, op_hit_log_path):
        ...
        try:
            ...
            self._run_repo_python_subprocess(
                ...
                error_prefix="float buffer unlabeled layer norm linear subprocess failed.",
            )
            ...
            self.assertIn("op=aten::native_layer_norm.buffer_width", op_hit_text)
            self.assertIn("op=aten::linear.family_unified_buffer_view", op_hit_text)
            self.assertNotIn("op=aten::layer_norm.fused_width", op_hit_text)
            ...
            if os.path.exists(materialize_log_path):
                ...
                self.assertNotIn(
                    "caller=layer_norm.output_only path=buffer_to_texture_via_staging",
                    materialize_log_text,
                )
                self.assertNotIn(
                    "caller=linear runtime= backing_label=layer_norm.output_only path=texture_to_buffer",
                    materialize_log_text,
                )
        finally:
            for path in (materialize_log_path, op_hit_log_path):
    ...
    def test_float_buffer_backbone_mlp_views_feed_linear_without_buffer_relayout(self):
        ...
        try:
            script = """
                ...
                torch.manual_seed(0)
                x_cpu = torch.randn(1, 601, 384, dtype=torch.float32)
                norm_weight_cpu = torch.randn(384, dtype=torch.float32)
                norm_bias_cpu = torch.randn(384, dtype=torch.float32)
                fc1_weight_cpu = torch.randn(1536, 384, dtype=torch.float32)
                ...
                x = x_cpu.to("vulkan")
                norm_weight = norm_weight_cpu.to("vulkan")
                norm_bias = norm_bias_cpu.to("vulkan")
                fc1_weight = fc1_weight_cpu.to("vulkan")
                fc1_bias = fc1_bias_cpu.to("vulkan")
                fc2_weight = fc2_weight_cpu.to("vulkan")
                fc2_bias = fc2_bias_cpu.to("vulkan")
            ...
            self._run_repo_python_subprocess(
                ...
                error_prefix="Float buffer backbone MLP relayout subprocess failed.",
            )
            ...
            if os.path.exists(materialize_log_path):
                ...
                self.assertNotIn(
                    "caller=linear runtime= backing_label=layer_norm.buffer_width path=buffer_relayout",
                    materialize_log_text,
                )
                self.assertNotIn(
                    "caller=linear runtime= backing_label=gelu path=buffer_relayout",
                    materialize_log_text,
                )
                self.assertNotIn(
                    "materialize_to_contiguous_buffer caller=linear runtime= backing_label=layer_norm.buffer_width",
                    materialize_log_text,
                )
                self.assertNotIn(
                    "materialize_to_contiguous_buffer caller=linear runtime= backing_label=gelu",
                    materialize_log_text,
                )
        finally:
            for path in (materialize_log_path, op_hit_log_path):
    ...
    def _run_batch_norm_inference_contract_generated_spec(self, spec_file_name):
        ...
        try:
            script = """
                ...
                def maybe_to_vulkan_input(tensor, case, use_vulkan):
                    if not use_vulkan:
                        return tensor
                    if not case.get("materialized_input", False):
                ...
                def run_batch_norm(case, tensors, use_vulkan):
                    ...
                    return F.batch_norm(
                        maybe_to_vulkan_input(x, case, use_vulkan),
                ...
                def run_case(case, expect_native_route):
                    ...
                    if expect_native_route:
                        ...
                        expected_contract_family = (
                            "contract_family="
                            + case.get("expected_contract_family", spec["family"])
                        )
                        expected_contract_tuple = (
                            "contract_tuple_id="
                            + case.get("expected_contract_tuple_id", spec["tuple_id"])
                        )
    ...
    def test_batch_norm_inference_contract_generated_buffer_float4d_spec(self):
        self._run_batch_norm_inference_contract_generated_spec(
            "batch_norm_inference_contract.json"
        )

    def test_batch_norm_inference_contract_generated_materialized_float4d_spec(self):
        self._run_batch_norm_inference_contract_generated_spec(
            "batch_norm_inference_materialized_contract.json"
        )
    ...
    def test_upsample_nearest2d_uint8_matches_cpu(self):
        torch.manual_seed(0)
        image = torch.randint(
            0, 256, (1, 3, 17, 23), dtype=torch.uint8, device="cpu")

        expected = F.interpolate(image, size=(420, 280), mode="nearest")
    ...
    def test_hwc_uint8_upload_roundtrip_matches_cpu(self):
        torch.manual_seed(0)
        image = torch.randint(
            0, 256, (17, 23, 3), dtype=torch.uint8, device="cpu")

        actual = image.to("vulkan").cpu()

        self.assertEqual(image.dtype, actual.dtype)
        self.assertEqual(image, actual)

    def test_hwc_uint8_padded_stride_upload_roundtrip_matches_cpu(self):
        torch.manual_seed(0)
        image_rgba = torch.randint(
            0, 256, (17, 23, 4), dtype=torch.uint8, device="cpu")
        image_rgb_view = image_rgba[:, :, :3]

        self.assertFalse(image_rgb_view.is_contiguous())
        self.assertEqual(image_rgb_view.stride(), (23 * 4, 4, 1))

        actual = image_rgb_view.to("vulkan").cpu()

        self.assertEqual(image_rgb_view.dtype, actual.dtype)
        self.assertEqual(image_rgb_view, actual)

    def test_upsample_nearest2d_uint8_repeated_cpu_readback_matches_cpu(self):
        torch.manual_seed(0)
        image = torch.randint(
            0, 256, (1, 3, 17, 23), dtype=torch.uint8, device="cpu")
    ...
    def test_min_all_matches_cpu(self):
        torch.manual_seed(0)
        image = torch.randn(3, 17, 23, device="cpu")

        expected = image.min()
        actual = image.to("vulkan").min().cpu()

        self.assertEqual(expected, actual)

    def test_min_max_all_buffer_view_match_cpu_without_relayout(self):
        log_name = "vulkan_min_max_buffer_view_materialize_test.log"
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        log_path = os.path.join(repo_root, log_name)
        if os.path.exists(log_path):
            os.remove(log_path)
        ...
        try:
            script = """
                ...
                torch.manual_seed(0)
                base_cpu = torch.rand(1, 1, 420, 280, dtype=torch.float32)
                view_cpu = base_cpu[0, 0]
                base_vulkan = base_cpu.to("vulkan")
                view_vulkan = base_vulkan[0, 0]
                ...
                print(float(view_vulkan.min().cpu() + view_vulkan.max().cpu()))
            """

            self._run_repo_python_subprocess(
                script,
                extra_env={"PYTORCH_VULKAN_MATERIALIZE_LOG": log_name},
                error_prefix="buffer-view min/max subprocess failed.",
            )

            if os.path.exists(log_path):
                with open(log_path, "r", encoding="utf-8") as log_file:
                    for line in log_file:
                        if "sizes=[420,280]" not in line:
                            continue
                        self.assertNotIn("kind=ensure_buffer_storage", line)
                        self.assertNotIn(
                            "kind=materialize_to_contiguous_buffer",
                            line,
                        )
        finally:
            if os.path.exists(log_path):
    ...
    def test_decoder_head_conv_transpose2d_runs_without_inference_mode(self):
        log_name = "decoder_head_conv_transpose2d_no_inference_mode_op_hit_test.log"
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        log_path = os.path.join(repo_root, log_name)
        if os.path.exists(log_path):
            os.remove(log_path)
        ...
        try:
            script = """
                ...
                torch.manual_seed(0)
                module_cpu = torch.nn.Sequential(
                    torch.nn.Conv2d(16, 8, kernel_size=1, stride=1, padding=0, bias=True),
                    torch.nn.ConvTranspose2d(8, 8, kernel_size=4, stride=4, bias=True),
                ).eval()
                module_vulkan = torch.nn.Sequential(
                    torch.nn.Conv2d(16, 8, kernel_size=1, stride=1, padding=0, bias=True),
                    torch.nn.ConvTranspose2d(8, 8, kernel_size=4, stride=4, bias=True),
                ).eval()
                module_vulkan.load_state_dict(module_cpu.state_dict())
                ...
                x_cpu = torch.randn(1, 16, 20, 20, dtype=torch.float32)
                actual = module_vulkan(x_cpu.to("vulkan")).cpu()
                hidden = F.conv2d(
                    x_cpu,
                    module_cpu[0].weight,
                ...
                weight = module_cpu[1].weight
                bias = module_cpu[1].bias
                n, in_channels, height, width = hidden.shape
                out_channels = weight.shape[1]
                kernel_h = weight.shape[2]
                kernel_w = weight.shape[3]
                expected = torch.empty(
                    (n, out_channels, height * kernel_h, width * kernel_w),
                    dtype=hidden.dtype,
                )
                    ...
                    for out_channel in range(out_channels):
                        for h in range(height):
                            for w in range(width):
                                patch = torch.zeros((kernel_h, kernel_w), dtype=hidden.dtype)
                                for in_channel in range(in_channels):
                                    patch += (
                                        hidden[batch, in_channel, h, w]
                                        * weight[in_channel, out_channel]
                                    )
                                expected[
                                    batch,
                                    out_channel,
                                    h * kernel_h : (h + 1) * kernel_h,
                                    w * kernel_w : (w + 1) * kernel_w,
                                ] = patch
                        expected[batch, out_channel].add_(bias[out_channel])
                torch.testing.assert_close(actual, expected, atol=1e-3, rtol=1e-3)
                print("ok")
            """
            ...
            self._run_repo_python_subprocess(
                script,
                extra_env={"PYTORCH_VULKAN_OP_HIT_LOG": log_name},
                error_prefix="Decoder-head conv_transpose2d subprocess failed.",
            )
            ...
            self.assertRegex(
                log_text,
                r"op=aten::convolution\.(buffer_float_transpose_exact_rearrange|buffer_float_transpose(_nonoverlap)?)",
            )
        finally:
            if os.path.exists(log_path):
                os.remove(log_path)
    ...
    def test_vits_resize1_conv_transpose2d_sparse_basis_matches_manual_nonoverlap(self):
        ...
        try:
            script = """
                ...
                for pos_y, pos_x in ((0, 0), (0, 13), (5, 7), (19, 19)):
                    x_cpu = torch.zeros(1, 96, 20, 20, dtype=torch.float32)
                    x_cpu[0, 0, pos_y, pos_x] = 1.0
                    actual = module_vulkan(x_cpu.to("vulkan")).cpu()

                    expected = torch.zeros_like(actual)
                    out_y = pos_y * 2
                    out_x = pos_x * 2
                    expected[0, :, out_y : out_y + 2, out_x : out_x + 2] = weight[0]
                    torch.testing.assert_close(actual, expected, atol=3e-4, rtol=3e-4)
                print("ok")
            """
    ...
    def test_diffusion_sdpa_contract_generated_spec(self):
        spec = _load_vulkan_contract_spec("diffusion_sdpa_contract.json")
        ...
        for _, case, expect_native_route in (
            ...
            with self.subTest(case=case["name"]):
                torch.manual_seed(7300 + len(case["name"]))
                query, key, value = make_tensors(case)
                query_vulkan = query.to("vulkan")
                key_vulkan = key.to("vulkan")
                value_vulkan = value.to("vulkan")
    ...
    def test_sdpa_execution_policy_contract_generated_spec(self):
        ...
        def make_tensors(case):
            dtype = dtype_for_case(case)
            query = torch.randn(
                *case.get("runtime_query_shape", case["query_shape"]),
                dtype=dtype,
            )
            key = torch.randn(
                *case.get("runtime_key_shape", case["key_shape"]),
                dtype=dtype,
            )
            value = torch.randn(
                *case.get("runtime_value_shape", case["value_shape"]),
                dtype=dtype,
            )
            return query, key, value
        ...
        for _, case, expect_native_route in (
            ...
            with self.subTest(case=case["name"]):
                torch.manual_seed(7400 + len(case["name"]))
                query, key, value = make_tensors(case)
                query_vulkan = query.to("vulkan")
                key_vulkan = key.to("vulkan")
                value_vulkan = value.to("vulkan")
    ...
    def test_scaled_dot_product_attention_multi_head_diffusion_matches_cpu(self):
        torch.manual_seed(0)
        query = torch.randn(1, 5, 640, 64)
        key = torch.randn(1, 5, 640, 64)
        value = torch.randn(1, 5, 640, 64)
    ...
    def test_scaled_dot_product_attention_diffusion_cross_504x2_matches_cpu(self):
        torch.manual_seed(0)
        query = torch.randn(1, 5, 504, 64)
        key = torch.randn(1, 5, 2, 64)
        value = torch.randn(1, 5, 2, 64)
    ...
    def test_scaled_dot_product_attention_diffusion_cross_126x2_matches_cpu(self):
        ...
        key = torch.randn(1, 10, 2, 64)
        value = torch.randn(1, 10, 2, 64)
    ...
    def test_scaled_dot_product_attention_diffusion_cross_20h35x2_matches_cpu(self):
        ...
        key = torch.randn(1, 20, 2, 64)
        value = torch.randn(1, 20, 2, 64)
    ...
    def test_scaled_dot_product_attention_direct_diffusion_10h126_matches_cpu(self):
        torch.manual_seed(0)
        query = torch.randn(1, 10, 126, 64)
        key = torch.randn(1, 10, 126, 64)
        value = torch.randn(1, 10, 126, 64)
    ...
    def test_scaled_dot_product_attention_direct_diffusion_20h12_matches_cpu(self):
        torch.manual_seed(0)
        query = torch.randn(1, 20, 12, 64)
        key = torch.randn(1, 20, 12, 64)
        value = torch.randn(1, 20, 12, 64)
    ...
    def test_vulkan_dispatch_tables_expose_backend_kernels(self):
        ...
        linear_table = torch._C._dispatch_dump_table("aten::linear")
        self.assertIn("CompositeImplicitAutograd", linear_table)
        self.assertIn("RegisterCompositeImplicitAutograd", linear_table)
        self.assertNotIn("Mm.cpp", linear_table)
    ...
    def test_vulkan_packed_weight_residency_prunes_expired_sources(self):
        log_name = "vulkan_packed_weight_residency_prune_test.log"
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        log_path = os.path.join(repo_root, log_name)
        if os.path.exists(log_path):
            os.remove(log_path)
        ...
        try:
            script = """
                ...
                torch.manual_seed(0)
                x = torch.randn(1, 4, 8, dtype=torch.float32).to("vulkan")

                def create_and_drop_context(seed: int):
                    torch.manual_seed(seed)
                    ...
                    y = torch.ops.vulkan_prepack.run_linear_context(x, context)
                    print(float(y.cpu().sum()))
                    del y, context, weight, bias
                    torch.ops.vulkan_prepack.synchronize()
                    gc.collect()

                with torch.inference_mode():
                    create_and_drop_context(1)
                    create_and_drop_context(2)
            """

            self._run_repo_python_subprocess(
                script,
                extra_env={
                    "PYTORCH_VULKAN_PACKED_WEIGHT_CACHE_LOG": log_name,
                },
                error_prefix="Packed-weight residency prune subprocess failed.",
            )
            ...
            self.assertRegex(
                log_text,
                r"packed_weight_residency: .* stores=2\s+evictions=[1-9][0-9]*",
            )
        finally:
            if os.path.exists(log_path):
                os.remove(log_path)

    def test_vulkan_linear_context_cache_prunes_expired_sources(self):
        log_name = "vulkan_linear_context_cache_prune_test.log"
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        log_path = os.path.join(repo_root, log_name)
        if os.path.exists(log_path):
            os.remove(log_path)
        ...
        try:
            ...
            self._run_repo_python_subprocess(
                script,
                extra_env={
                    "PYTORCH_VULKAN_LINEAR_CACHE_LOG": log_name,
                },
                error_prefix="Linear-context cache prune subprocess failed.",
            )
            ...
            self.assertRegex(
                log_text,
                r"linear_cache: .* stores=2\s+evictions=[1-9][0-9]*",
            )
        finally:
            if os.path.exists(log_path):
                os.remove(log_path)
    ...
    def _make_vulkan_vision_stack_shape_plan_fixture(
        self,
        token_count,
        blocks=1,
        embed_dim=384,
        num_heads=6,
        hidden_dim=768,
        label_prefix="vision.synthetic.stack.shape_plan",
    ):
        torch.manual_seed(0)
        norm_eps = 1.0e-6
        self.assertEqual(embed_dim % num_heads, 0)
        ...
        for block_index in range(blocks):
            contexts.append(
                torch.ops.vulkan_prepack.create_vision_backbone_block_context(
                    ...
                    randn(embed_dim),
                    randn(embed_dim),
                    f"{label_prefix}.block{block_index}",
                )
            )
        stack_context = torch.ops.vulkan_prepack.create_vision_backbone_stack_context(
            contexts,
    ...
    def test_vulkan_vision_stack_shape_plan_accepts_synthetic_601(self):
        contexts, stack_context, x = self._make_vulkan_vision_stack_shape_plan_fixture(
            601,
            blocks=2,
            label_prefix="vision.synthetic.stack.generalization",
        )
        ...
        self.assertTrue(any("tokens=601" in key for key in keys))
        readiness = torch.ops.vulkan_prepack.stack_shape_plan_readiness()
        self.assertTrue(
            any(
                "tokens=601" in row
                and "hidden=384" in row
                and "heads=6" in row
                and "head_dim=64" in row
                and "blocks=2" in row
    ...
    def test_vulkan_vision_stack_shape_plan_keys_are_token_specific(self):
        _, stack_context, x_601 = self._make_vulkan_vision_stack_shape_plan_fixture(
            601,
            label_prefix="vision.synthetic.stack.tokens",
        )
        _, _, x_607 = self._make_vulkan_vision_stack_shape_plan_fixture(
            607,
            label_prefix="vision.synthetic.stack.tokens",
        )
        ...
        keys = torch.ops.vulkan_prepack.stack_shape_plan_keys()
        self.assertTrue(any("tokens=601" in key for key in keys))
        self.assertTrue(any("tokens=607" in key for key in keys))
        counters = torch.ops.vulkan_prepack.stack_shape_plan_counters()
        self.assertGreaterEqual(counters[1], 2)
        self.assertGreaterEqual(counters[2], 1)
        self.assertGreaterEqual(counters[4], 3)
        self.assertGreaterEqual(counters[5], 1)
        self.assertGreaterEqual(counters[6], 1)

    def test_vulkan_vision_stack_unsupported_head_dim_avoids_q4_direct_path(self):
        _, stack_context, x = self._make_vulkan_vision_stack_shape_plan_fixture(
            ...
            num_heads=5,
            hidden_dim=800,
            label_prefix="vision.synthetic.stack.head_dim80",
        )

        torch.ops.vulkan_prepack.reset_fallback_counters()
        with self.assertRaisesRegex(RuntimeError, "KnownBadGenericSdpa|head"):
            with torch.inference_mode():
                torch.ops.vulkan_prepack.run_vision_backbone_stack_context(
                    x,
                    stack_context,
        ...
        stack_attention = torch.ops.vulkan_prepack.stack_attention_counters()
        attention_plan = torch.ops.vulkan_prepack.attention_plan_counters()
        self.assertEqual(stack_attention[1], 0)
        self.assertEqual(attention_plan[10], 0)
        phase_counters = torch.ops.vulkan_prepack.fallback_phase_counters()
        self.assertEqual(phase_counters[3], 0)

    def test_vulkan_vision_stack_unsupported_dtype_rejects(self):
        _, stack_context, x = self._make_vulkan_vision_stack_shape_plan_fixture(
            257,
            label_prefix="vision.synthetic.stack.dtype_reject",
        )
        x_half = x.cpu().half().to("vulkan")
        ...
        stack_counters = torch.ops.vulkan_prepack.vision_stack_owner_counters()
        stack_attention = torch.ops.vulkan_prepack.stack_attention_counters()
        self.assertGreaterEqual(stack_counters[6], 1)
        self.assertEqual(stack_attention[1], 0)

    def test_vulkan_vision_stack_requested_intermediates_synthetic(self):
        contexts, stack_context, x = self._make_vulkan_vision_stack_shape_plan_fixture(
            607,
            blocks=3,
            label_prefix="vision.synthetic.stack.intermediates",
        )
        ...
        with torch.inference_mode():
            ...
            y2 = torch.ops.vulkan_prepack.run_vision_backbone_block_context(
                y1,
                contexts[2],
            )
            stack_outputs = torch.ops.vulkan_prepack.run_vision_backbone_stack_context(
        ...
        self._assert_outputs_close(
            stack_outputs[1].cpu(), y2.cpu(), atol=5e-3, rtol=5e-3
        )
    ...
    def test_vulkan_vision_stack_execution_manifest_reports_capture_blocker(self):
        ...
        context = torch.ops.vulkan_prepack.create_vision_backbone_block_context(
            ...
            randn(embed_dim),
            randn(embed_dim),
            "depth.dino.backbone.stack.manifest.block0",
        )
        stack_context = torch.ops.vulkan_prepack.create_vision_backbone_stack_context(
            [context],
            num_heads,
        ...
        torch.ops.vulkan_prepack.reset_stack_execution_manifest()
        torch.ops.vulkan_prepack.reset_fallback_phase_counters()

        with torch.inference_mode():
            outputs = torch.ops.vulkan_prepack.run_vision_backbone_stack_context(
                x,
                stack_context,
                [0],
            )
        ...
        rows = torch.ops.vulkan_prepack.stack_execution_manifest()
        ...
        for phase in (
            ...
            "phase=fc2",
            "phase=residual2",
            "phase=intermediate_capture",
        ):
            self.assertTrue(any(phase in row for row in rows), phase)
        self.assertTrue(
            any(
                "op=vulkan_prepack::vision_stack_attention_direct" in row
                and "safe_to_capture=0" in row
                for row in rows
            )
        )
        self.assertTrue(
            any(
                "phase=intermediate_capture" in row
                and "escapes_stack=1" in row
                and "requested_intermediate=1" in row
                for row in rows
            )
        )
        readiness = torch.ops.vulkan_prepack.stack_capture_readiness()
        ...
        self.assertEqual(readiness[4], 1)
        self.assertEqual(readiness[5], 1)
        self.assertEqual(readiness[6], 1)
        self.assertEqual(readiness[7], 1)
    ...
    def test_vulkan_attention_runtime_public_entry_half_falls_back_from_replay(self):
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        sdpa_log_name = "vulkan_attention_runtime_public_half_fallback_test.log"
        sdpa_log_path = os.path.join(repo_root, sdpa_log_name)
        if os.path.exists(sdpa_log_path):
            os.remove(sdpa_log_path)
        ...
        try:
            script = """
                ...
                q_ref = (torch.randn(batch, heads, target_len, head_dim) * 0.25).to(torch.float16).float()
                k_ref = (torch.randn(batch, heads, source_len, head_dim) * 0.25).to(torch.float16).float()
                v_ref = (torch.randn(batch, heads, source_len, value_dim) * 0.25).to(torch.float16).float()
                q = q_ref.to(torch.float16).to("vulkan")
                k = k_ref.to(torch.float16).to("vulkan")
                v = v_ref.to(torch.float16).to("vulkan")

                previous = torch.ops.vulkan_prepack.swap_runtime_label(
                    "llama.self_attn.attention_runtime.capture.half_fallback"
                )
                try:
                    with torch.inference_mode():
                        actual = F.scaled_dot_product_attention(
                        ...
                        if actual.shape != q.shape:
                            raise RuntimeError("Half SDPA fallback returned an unexpected shape")
                        if not torch.isfinite(actual.cpu().float()).all():
                            raise RuntimeError("Half SDPA fallback produced non-finite values")
                finally:
                    torch.ops.vulkan_prepack.swap_runtime_label(previous)

                print(float(actual.cpu().float().sum()))
            """
            ...
            self._run_repo_python_subprocess(
                ...
                error_prefix="Half SDPA runtime public-entry fallback subprocess failed.",
            )
            ...
            with open(sdpa_log_path, "r", encoding="utf-8") as log_file:
                sdpa_log_text = log_file.read()
            self.assertIn(
                "math_vulkan_entry result=enter reason=ok",
                sdpa_log_text,
            )
            self.assertNotIn(
                "public_vulkan_entry result=replay reason=attention_runtime",
    ...
    def test_vulkan_vision_decoder_preprocess_head_compiled_session(self):
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        ...
        try:
            script = """
                ...
                head_context = torch.ops.vulkan_prepack.create_vision_decoder_head_context(
                    ...
                    True,
                    "depth.decoder.preprocess.test.head",
                )
                ...
                def run_reference(tokens):
                    ...
                    for idx, token in enumerate(tokens):
                        ...
                        if resize_contexts[idx] is not None:
                            layer = torch.ops.vulkan_prepack.run_tconv2d_context(
                                layer, resize_contexts[idx]
                            )
                        layer = torch.ops.vulkan_prepack.run_conv2d_context(
    ...
    def test_vulkan_depth_anything_v2_compiled_session_bridge(self):
        ...
        try:
            script = """
                ...
                contexts = [
                    make_backbone_context("depth.dino.backbone.full.block0"),
                    make_backbone_context("depth.dino.backbone.full.block1"),
                    make_backbone_context("depth.dino.backbone.full.block2"),
                    make_backbone_context("depth.dino.backbone.full.block3"),
                ]
                capture_indices = [0, 1, 2, 3]
                norm_context = torch.ops.vulkan_prepack.create_layernorm_context(
                    randn(embed_dim),
                ...
                try:
                    with torch.inference_mode():
                        y0 = torch.ops.vulkan_prepack.run_depth_anything_v2_compiled_session_bridge(
                            x0,
                            contexts,
                            capture_indices,
                            [embed_dim],
                        ...
                        y1 = torch.ops.vulkan_prepack.run_depth_anything_v2_compiled_session_bridge(
                            x1,
                            contexts,
                            capture_indices,
                            [embed_dim],
    ...
    def test_vulkan_depth_anything_v2_image_compiled_session_bridge(self):
        ...
        try:
            script = """
                ...
                contexts = [
                    make_backbone_context("depth.dino.backbone.image.block0"),
                    make_backbone_context("depth.dino.backbone.image.block1"),
                    make_backbone_context("depth.dino.backbone.image.block2"),
                    make_backbone_context("depth.dino.backbone.image.block3"),
                ]
                capture_indices = [0, 1, 2, 3]
                norm_context = torch.ops.vulkan_prepack.create_layernorm_context(
                    randn(embed_dim),
                ...
                def make_tokens(image):
                    ...
                    tokens = torch.cat(
                        (
                            prefix_token.expand(tokens.shape[0], -1, -1),
                            tokens + patch_pos_encoding,
                        ),
                        dim=1,
                    )
                    if tokens.shape[0] == 1:
                ...
                x0 = torch.randn(1, 3, image_h, image_w, dtype=torch.float32).to("vulkan")
                x1 = torch.randn(1, 3, image_h, image_w, dtype=torch.float32).to("vulkan")

                previous = torch.ops.vulkan_prepack.swap_runtime_label(
                    f"depth.vision.capture.{image_h}x{image_w}"
                )
                try:
                    with torch.inference_mode():
                        expected0 = torch.ops.vulkan_prepack.run_depth_anything_v2_compiled_session_bridge(
                            make_tokens(x0),
                            contexts,
                            capture_indices,
                            [embed_dim],
                        ...
                        actual0 = torch.ops.vulkan_prepack.run_depth_anything_v2_image_compiled_session_bridge(
                            x0,
                            patch_embed_context,
                            prefix_token,
                            patch_pos_encoding,
                        ...
                        expected1 = torch.ops.vulkan_prepack.run_depth_anything_v2_compiled_session_bridge(
                            make_tokens(x1),
                            contexts,
                            capture_indices,
                            [embed_dim],
                        ...
                        actual1 = torch.ops.vulkan_prepack.run_depth_anything_v2_image_compiled_session_bridge(
                            x1,
                            patch_embed_context,
                            prefix_token,
                            patch_pos_encoding,
    ...
    def test_vulkan_execution_graph_root_keeps_distinct_vision_bundles(self):
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        graph_log_name = "vulkan_execution_graph_root_distinct_vision_bundle_test.log"
        graph_log_path = os.path.join(repo_root, graph_log_name)
        if os.path.exists(graph_log_path):
            os.remove(graph_log_path)
        ...
        try:
            script = """
                ...
                def make_decoder_context(label: str):
                    return torch.ops.vulkan_prepack.create_vision_decoder_fusion_block_context(
                        torch.randn(channels, channels, 3, 3, dtype=torch.float32),
                        torch.randn(channels, dtype=torch.float32),
                        torch.randn(channels, channels, 3, 3, dtype=torch.float32),
                ...
                backbone_context_a = make_backbone_context("depth.dino.backbone.block.bundle_collision.a")
                decoder_context_a = make_decoder_context("depth.decoder.block.bundle_collision.a")
                backbone_context_b = make_backbone_context("depth.dino.backbone.block.bundle_collision.b")
                decoder_context_b = make_decoder_context("depth.decoder.block.bundle_collision.b")
                ...
                previous = torch.ops.vulkan_prepack.swap_runtime_label(
                    "depth.vision.shared.bundle_collision"
                )
                try:
                    with torch.inference_mode():
                        torch.ops.vulkan_prepack.run_vision_backbone_decoder_replay_bundle_bridge(
                            backbone_input_a,
                            backbone_context_a,
                            decoder_input_a,
                            decoder_skip_a,
                        ...
                        if not torch.isfinite(y_backbone_b.cpu()).all():
                            raise RuntimeError("Backbone bundle output produced non-finite values")
                        if not torch.isfinite(y_decoder_b.cpu()).all():
                            raise RuntimeError("Decoder bundle output produced non-finite values")
                finally:
                    torch.ops.vulkan_prepack.swap_runtime_label(previous)

                print(float(y_backbone_b.cpu().sum() + y_decoder_b.cpu().sum()))
            """

            self._run_repo_python_subprocess(
                script,
                extra_env={"PYTORCH_VULKAN_INFERENCE_GRAPH_LOG": graph_log_name},
                error_prefix="Distinct shared-root vision bundle subprocess failed.",
            )
            ...
            self.assertEqual(
                graph_log_text.count(
                    "execution_graph_root event=store allocation_label=depth.vision.shared.bundle_collision.graph"
                ),
                1,
            )
            self.assertEqual(
                graph_log_text.count(
                    "execution_graph_root event=bundle_store allocation_label=depth.vision.shared.bundle_collision.graph"
                ),
                2,
            )
            self.assertEqual(
                graph_log_text.count(
                    "inference_replay event=store kind=ExecutionGraphBundle allocation_label=depth.vision.shared.bundle_collision.graph.vision.backbone_decoder.replay"
                ),
                2,
            )
        finally:
    ...
    def test_vulkan_vision_backbone_stack_replay_bundle_bridge(self):
        ...
        try:
            script = """
                ...
                previous = torch.ops.vulkan_prepack.swap_runtime_label(
                    "depth.vision.stack.norm.capture.17x32"
                )
                try:
                    with torch.inference_mode():
                        y0_norm = torch.ops.vulkan_prepack.run_vision_backbone_stack_norm_replay_bundle_bridge(
                            x0, contexts, capture_indices, [embed_dim], norm_context
                        )
                        y0_norm_cpu = [tensor.cpu().clone() for tensor in y0_norm]
                        y1_norm = torch.ops.vulkan_prepack.run_vision_backbone_stack_norm_replay_bundle_bridge(
                ...
                for actual, expected in zip(y0_norm_cpu, expected0_norm):
                    if not torch.allclose(actual, expected, atol=1e-4, rtol=1e-4):
                        raise RuntimeError("Backbone stack norm replay bundle warmup output mismatch")
            ...
            self.assertIn("normctx", graph_log_text)
        finally:
            if os.path.exists(graph_log_path):
                os.remove(graph_log_path)
    ...
    def test_vulkan_capability_profile_minimum_clamps_compiled_session_layout(
        self,
    ):
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        op_hit_log_name = "vulkan_capability_profile_min_compiled_session_op.log"
        session_log_name = "vulkan_capability_profile_min_compiled_session.log"
        op_hit_log_path = os.path.join(repo_root, op_hit_log_name)
        session_log_path = os.path.join(repo_root, session_log_name)
        for path in (op_hit_log_path, session_log_path):
            if os.path.exists(path):
                os.remove(path)
        ...
        try:
            script = """
                ...
                contexts = [
                    make_context("depth.profile.compiled.block0"),
                    make_context("depth.profile.compiled.block1"),
                    make_context("depth.profile.compiled.block2"),
                ]
                capture_indices = [0, 2]
                x0 = torch.randn(
                    1, token_count, embed_dim, dtype=torch.float32
                ).to("vulkan")
                x1 = torch.randn(
                    1, token_count, embed_dim, dtype=torch.float32
                ).to("vulkan")
                ...
                torch.ops.vulkan_prepack.reset_fallback_counters()
                previous = torch.ops.vulkan_prepack.swap_runtime_label(
                    "depth.profile.stack.compiled.capture.17x32"
                )
                try:
                    with torch.inference_mode():
                        y0 = torch.ops.vulkan_prepack.run_vision_backbone_stack_compiled_session_bridge(
                ...
                for actual, expected in zip(y0_cpu, expected0):
                    if not torch.allclose(actual, expected, atol=1e-4, rtol=1e-4):
                        raise RuntimeError("Profile compiled session warmup mismatch")
                for actual, expected in zip(y1_cpu, expected1):
                    if not torch.allclose(actual, expected, atol=1e-4, rtol=1e-4):
                        raise RuntimeError("Profile compiled session replay mismatch")
                assert torch.ops.vulkan_prepack.cpu_fallback_count() == 0
                print(float(
                    sum(t.sum() for t in y0_cpu)
                    + sum(t.sum() for t in y1_cpu)
            ...
            self._run_repo_python_subprocess(
                ...
                extra_env={
                    "PYTORCH_VULKAN_CAPABILITY_PROFILE": "vk_min_1_1_compute",
                    "PYTORCH_VULKAN_COMPILED_SESSION_LOG": session_log_name,
                    "PYTORCH_VULKAN_OP_HIT_LOG": op_hit_log_name,
                },
                error_prefix=(
                    "Minimum-profile compiled-session subprocess failed."
                ),
            )
            ...
            self.assertIn(
                "op=vulkan_prepack::"
                "run_vision_backbone_stack_compiled_session.replay_warmup",
                op_hit_text,
            )
            self.assertIn(
                "op=vulkan_prepack::"
                "run_vision_backbone_stack_compiled_session.replay",
                op_hit_text,
            )

            self.assertTrue(os.path.exists(session_log_path))
            with open(session_log_path, "r", encoding="utf-8") as log_file:
                session_log_text = log_file.read()
            for key_field in (
                "|bf16=0",
                "|i8=0",
                "|sb8=0",
                ...
                self.assertIn(key_field, session_log_text)
            self.assertIn("width_alignment=1", session_log_text)
            self.assertIn("pad_width=0", session_log_text)
            self.assertIn("reason=buffer_first_width_packed", session_log_text)
            self.assertNotIn(
                "buffer_first_width_packed_cooperative_matrix_ready",
                session_log_text,
            )
        finally:
            for path in (op_hit_log_path, session_log_path):
    ...
    def test_large_generic_buffer_linear_uses_conservative_kernel(self):
        ...
        try:
            ...
            self._run_repo_python_subprocess(
                script,
                extra_env={"PYTORCH_VULKAN_OP_HIT_LOG": log_name},
                error_prefix="Large generic buffer linear tiled subprocess failed.",
            )
    ...
    def test_dinov2_attention_linear_and_matmul_run_without_inference_mode(self):
        ...
        try:
            script = """
                ...
                torch.manual_seed(0)
                x_cpu = torch.randn(1, 65, 384, dtype=torch.float32)
                w_cpu = torch.randn(1152, 384, dtype=torch.float32, requires_grad=True)
                b_cpu = torch.randn(1152, dtype=torch.float32, requires_grad=True)
                ...
                try:
                    qkv = F.linear(x, w, b)
                    q, k, _ = qkv.reshape(1, 65, 3, 6, 64).permute(2, 0, 3, 1, 4)
                    y = q @ k.transpose(-2, -1)
                finally:
                    torch.ops.vulkan_prepack.swap_runtime_label(previous)

                expected_qkv = F.linear(x_cpu, w_cpu, b_cpu).reshape(
                    1, 65, 3, 6, 64
                ).permute(2, 0, 3, 1, 4)
                expected = expected_qkv[0] @ expected_qkv[1].transpose(-2, -1)
                torch.testing.assert_close(y.cpu(), expected, atol=3e-3, rtol=3e-3)
                print("ok")
            """
            ...
            self._run_repo_python_subprocess(
                script,
                extra_env={"PYTORCH_VULKAN_OP_HIT_LOG": log_name},
                error_prefix="Dinov2 attention linear subprocess failed.",
            )
            ...
            self.assertIn("op=aten::linear.family_unified_buffer_view", log_text)
            self.assertIn("op=aten::linear.buffer_float", log_text)
            self.assertIn("op=aten::bmm", log_text)
            self.assertNotRegex(log_text, r"(?m)^op=aten::linear$")
        finally:
            if os.path.exists(log_path):
                os.remove(log_path)

    def test_dinov2_attention_qkv_buffer_views_stay_on_buffer_path(self):
        materialize_log_name = "dinov2_attention_qkv_buffer_views_materialize_test.log"
        op_hit_log_name = "dinov2_attention_qkv_buffer_views_op_hit_test.log"
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        materialize_log_path = os.path.join(repo_root, materialize_log_name)
        op_hit_log_path = os.path.join(repo_root, op_hit_log_name)
        for path in (materialize_log_path, op_hit_log_path):
        ...
        try:
            script = """
                ...
                norm_weight_cpu = torch.randn(384, dtype=torch.float32)
                norm_bias_cpu = torch.randn(384, dtype=torch.float32)
                qkv_weight_cpu = torch.randn(1152, 384, dtype=torch.float32)
                qkv_bias_cpu = torch.randn(1152, dtype=torch.float32)
                proj_weight_cpu = torch.randn(384, 384, dtype=torch.float32)
                proj_bias_cpu = torch.randn(384, dtype=torch.float32)

                def run_attention(x, norm_weight, norm_bias, qkv_weight, qkv_bias, proj_weight, proj_bias):
                    x = F.layer_norm(x, (384,), norm_weight, norm_bias, 1.0e-6)
                    qkv = F.linear(x, qkv_weight, qkv_bias)
                    qkv = qkv.reshape(1, 65, 3, 6, 64).permute(2, 0, 3, 1, 4)
                    q, k, v = qkv[0] * (1.0 / math.sqrt(64.0)), qkv[1], qkv[2]
                ...
                norm_weight = norm_weight_cpu.to("vulkan")
                norm_bias = norm_bias_cpu.to("vulkan")
                qkv_weight = qkv_weight_cpu.to("vulkan")
                qkv_bias = qkv_bias_cpu.to("vulkan")
                ...
                torch.testing.assert_close(actual, expected, atol=4e-3, rtol=1.2e-2)
                print(float(actual.sum()))
            """
            ...
            self._run_repo_python_subprocess(
                ...
                error_prefix="Dinov2 attention buffer-view subprocess failed.",
            )
            ...
            self.assertIn("op=aten::native_layer_norm.buffer_width", op_hit_text)
            self.assertIn("op=aten::bmm.buffer_float", op_hit_text)
            self.assertIn("op=aten::_softmax.buffer_lastdim", op_hit_text)
            self.assertIn("op=aten::view.buffer_preserve_padded_reshape", op_hit_text)
            self.assertNotIn("op=aten::view.texture_metadata_reshape", op_hit_text)

            if os.path.exists(materialize_log_path):
                with open(materialize_log_path, "r", encoding="utf-8") as log_file:
                    materialize_log_text = log_file.read()

                self.assertNotIn("caller=bmm.pack", materialize_log_text)
                self.assertNotIn(
                    "caller=bmm runtime= backing_label=unlabeled path=image_layout_convert_width",
                    materialize_log_text,
                )
        finally:
            for path in (materialize_log_path, op_hit_log_path):
    ...
    def test_low_channel_bilinear_upsample_large_output_matches_cpu(self):
        op_hit_log_name = "low_channel_texture_bilinear_upsample_op_hit_test.log"
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        op_hit_log_path = os.path.join(repo_root, op_hit_log_name)
        for path in (op_hit_log_path,):
            if os.path.exists(path):
                os.remove(path)
        ...
        try:
            script = """
                ...
                torch.manual_seed(0)
                x_cpu = torch.rand(1, 3, 70, 70, dtype=torch.float32)
                weight = torch.rand(1, 3, 1, 1, dtype=torch.float32)
                bias = torch.rand(1, dtype=torch.float32)
                bilinear_input_cpu = F.conv2d(x_cpu, weight, bias)
                expected = F.interpolate(
                    bilinear_input_cpu,
                    size=(280, 280),
                    mode="bilinear",
                ...
                x_vulkan = x_cpu.to("vulkan")
                bilinear_input_vulkan = F.conv2d(
                    x_vulkan,
                    weight.to("vulkan"),
                    bias.to("vulkan"),
                )
            ...
            self._run_repo_python_subprocess(
                script,
                extra_env={"PYTORCH_VULKAN_OP_HIT_LOG": op_hit_log_name},
                error_prefix="low-channel bilinear upsample subprocess failed.",
            )
            ...
            self.assertIn("op=aten::upsample_bilinear2d.buffer_float", op_hit_text)
        finally:
            for path in (op_hit_log_path,):
                if os.path.exists(path):
                    os.remove(path)
    ...
    def test_dinov2_positional_reshape_runs_without_inference_mode(self):
        script = """
            ...
            torch.manual_seed(0)
            patch_pos_embed_cpu = torch.randn(1, 64, 32, dtype=torch.float32)
            patch_pos_embed = patch_pos_embed_cpu.to("vulkan")

            y = patch_pos_embed.reshape(1, 8, 8, 32).permute(0, 3, 1, 2)
            expected = patch_pos_embed_cpu.reshape(1, 8, 8, 32).permute(0, 3, 1, 2)
        ...
        self._run_repo_python_subprocess(
            script,
            error_prefix="Dinov2 positional reshape subprocess failed.",
        )
