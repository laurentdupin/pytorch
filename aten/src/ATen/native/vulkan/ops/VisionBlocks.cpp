...
bool selected_segment_block_budget_exceeded(
    const StackOwnedCommandBufferSegmentPlan& segment_plan) {
  if (segment_plan.selected_segment_count == 0u) {
    return false;
  }
  size_t segment_start = 0u;
  for (size_t segment_index = 0u;
...
StackOwnedCommandBufferSegmentPlan stack_dispatch_budget_prefix_segment_plan(
  ...
  result.coverage = result.segment_ends.size() > result.selected_segment_count
      ? "prefix"
      : "full";
  result.status = "dispatch_budget_prefix_plan_available_behavior_canary";
  result.fail_reason = result.segment_ends.size() > result.selected_segment_count
      ? "dispatch_budget_candidate_scope_limit_exceeded_prefix_selected"
      : "none";
...
std::vector<Tensor> run_vision_backbone_stack_context_impl(
  ...
  const bool dispatch_budget_prefix3_tail_canary_requested =
  ...
  for (size_t block_idx = 0u; block_idx < context->blocks().size(); ++block_idx) {
    if (segmented_stack_owned_command_buffer_canary && !planned_recording_scope) {
      ...
      const bool context_tail_segment =
          segmented_prefix_canary_requested && !selected_external_segment &&
          segmented_stack_owned_command_buffer_segment_index <
              segmented_stack_owned_command_buffer_plan.segment_ends.size();
      if (selected_external_segment || context_tail_segment) {
        const bool context_tail_to_exit_segment =
            dispatch_budget_prefix3_tail_canary_requested &&
            !selected_external_segment;
        const size_t segment_end = context_tail_to_exit_segment
            ? context->blocks().size() - 1u
