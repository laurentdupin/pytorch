...
const char* stack_owned_command_buffer_segmented_canary_mode() {
  const char* env =
  ...
  if (
      ...
      mode == "segmented_stack_dispatch_prefix_to_exit") {
    return env;
  }
  return "none";
}
...
bool stack_owned_command_buffer_segmented_dispatch_prefix_canary_enabled() {
  return std::string(stack_owned_command_buffer_segmented_canary_mode()) ==
      "segmented_stack_dispatch_prefix_to_exit";
}
...
StackOwnedCommandBufferSegmentPlan stack_owned_command_buffer_segment_plan(
  ...
  plan.selected_segment_count = plan.segment_ends.size();
  plan.coverage = "full";
...
bool selected_segment_dispatch_budget_exceeded(
    const VulkanVisionStackShapePlan* const plan,
    const StackOwnedCommandBufferSegmentPlan& segment_plan) {
  if (!plan || segment_plan.selected_segment_count == 0u) {
    return false;
  }
  size_t segment_start = 0u;
  for (size_t segment_index = 0u;
       segment_index < segment_plan.selected_segment_count;
       ++segment_index) {
...
StackOwnedCommandBufferSegmentPlan stack_owned_command_buffer_dispatch_budget_plan(
    const VulkanVisionStackShapePlan* const plan,
    const size_t block_count) {
  StackOwnedCommandBufferSegmentPlan segment_plan;
  if (!plan) {
    segment_plan.status = "segment_plan_rejected_behavior_neutral";
    segment_plan.fail_reason = "shape_plan_missing";
    return segment_plan;
  }
  if (block_count <= 2u) {
    segment_plan.status = "segment_plan_rejected_behavior_neutral";
    segment_plan.fail_reason = "segment_scope_too_small";
    return segment_plan;
  }

  size_t segment_start = 0u;
  while (segment_start < block_count) {
    size_t segment_end = segment_start;
    if (
        planned_segment_dispatch_count(plan, segment_start, segment_end) >
        kSegmentedOwnedCommandBufferDispatchLimit) {
      segment_plan.segment_ends.clear();
      segment_plan.status = "segment_plan_rejected_behavior_neutral";
      segment_plan.fail_reason = "segment_planned_dispatch_limit_exceeded";
      return segment_plan;
    }
    while (
        segment_end + 1u < block_count &&
        segment_end + 1u - segment_start + 1u <=
            kSegmentedOwnedCommandBufferBlockLimit &&
        planned_segment_dispatch_count(plan, segment_start, segment_end + 1u) <=
            kSegmentedOwnedCommandBufferDispatchLimit) {
      ++segment_end;
    }
    segment_plan.segment_ends.push_back(segment_end);
    segment_start = segment_end + 1u;
  }

  if (segment_plan.segment_ends.empty()) {
    segment_plan.status = "segment_plan_rejected_behavior_neutral";
    segment_plan.fail_reason = "no_segment_boundaries";
    return segment_plan;
  }

  segment_plan.selected_segment_count = std::min(
      segment_plan.segment_ends.size(),
      kSegmentedOwnedCommandBufferScopeLimit);
  segment_plan.coverage =
      segment_plan.selected_segment_count == segment_plan.segment_ends.size()
      ? "full"
      : "prefix";
  segment_plan.status =
      segment_plan.coverage == std::string("full")
      ? "segment_dispatch_budget_plan_available_behavior_canary"
      : "segment_dispatch_budget_prefix_plan_available_behavior_canary";
  segment_plan.fail_reason =
      segment_plan.coverage == std::string("full")
      ? "none"
      : "segment_scope_limit_exceeded_dispatch_prefix_selected";
  return segment_plan;
}
...
void append_optional_cpu_snapshot_for_unpack(
    c10::impl::GenericList& unpacked,
    const c10::impl::GenericList& unpack_sources,
    const uint32_t index,
    const char* reason) {
  const auto tensor = get_optional_tensor(unpack_sources, index);
  if (tensor.has_value()) {
    unpacked.emplace_back(cpu_snapshot_for_unpack(*tensor, reason));
  } else {
    unpacked.emplace_back(std::optional<Tensor>{});
  }
}
...
VisionBackboneBlockContext::VisionBackboneBlockContext(
    ...
    : cache_id_(
      ...
      num_heads_(num_heads),
      fc1_hidden_dim_(fc1_weight.size(0)),
  ...
  unpack_sources_.emplace_back(norm1_weight);
  unpack_sources_.emplace_back(norm1_bias);
  unpack_sources_.emplace_back(norm1_eps);
  unpack_sources_.emplace_back(qkv_weight);
  if (qkv_bias.has_value()) {
    unpack_sources_.emplace_back(*qkv_bias);
  } else {
    unpack_sources_.emplace_back(std::optional<Tensor>{});
  }
  if (attention_bias.has_value()) {
    unpack_sources_.emplace_back(*attention_bias);
  } else {
    unpack_sources_.emplace_back(std::optional<Tensor>{});
  }
  unpack_sources_.emplace_back(num_heads_);
  unpack_sources_.emplace_back(proj_weight);
  if (proj_bias.has_value()) {
    unpack_sources_.emplace_back(*proj_bias);
  } else {
    unpack_sources_.emplace_back(std::optional<Tensor>{});
  }
  if (ls1_gamma.has_value()) {
    unpack_sources_.emplace_back(*ls1_gamma);
  } else {
    unpack_sources_.emplace_back(std::optional<Tensor>{});
  }
  unpack_sources_.emplace_back(norm2_weight);
  unpack_sources_.emplace_back(norm2_bias);
  unpack_sources_.emplace_back(norm2_eps);
  unpack_sources_.emplace_back(fc1_weight);
  if (fc1_bias.has_value()) {
    unpack_sources_.emplace_back(*fc1_bias);
  } else {
    unpack_sources_.emplace_back(std::optional<Tensor>{});
  }
  unpack_sources_.emplace_back(fc2_weight);
  if (fc2_bias.has_value()) {
    unpack_sources_.emplace_back(*fc2_bias);
  } else {
    unpack_sources_.emplace_back(std::optional<Tensor>{});
  }
  if (ls2_gamma.has_value()) {
    unpack_sources_.emplace_back(*ls2_gamma);
  } else {
    unpack_sources_.emplace_back(std::optional<Tensor>{});
  }
...
const c10::impl::GenericList VisionBackboneBlockContext::unpack() const {
  ...
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::Norm1Weight).toTensor(),
      "unpack_norm1_weight_readback"));
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::Norm1Bias).toTensor(),
      "unpack_norm1_bias_readback"));
  unpacked.emplace_back(unpack_sources_.get(Unpacked::Norm1Eps).toDouble());
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::QkvWeight).toTensor(),
      "unpack_qkv_weight_readback"));
  append_optional_cpu_snapshot_for_unpack(
      unpacked,
      unpack_sources_,
      Unpacked::QkvBias,
      "unpack_qkv_bias_readback");
  append_optional_cpu_snapshot_for_unpack(
      unpacked,
      unpack_sources_,
      Unpacked::AttentionBias,
      "unpack_attention_bias_readback");
  unpacked.emplace_back(unpack_sources_.get(Unpacked::NumHeads).toInt());
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::ProjWeight).toTensor(),
      "unpack_proj_weight_readback"));
  append_optional_cpu_snapshot_for_unpack(
      unpacked,
      unpack_sources_,
      Unpacked::ProjBias,
      "unpack_proj_bias_readback");
  append_optional_cpu_snapshot_for_unpack(
      unpacked,
      unpack_sources_,
      Unpacked::Ls1Gamma,
      "unpack_ls1_gamma_readback");
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::Norm2Weight).toTensor(),
      "unpack_norm2_weight_readback"));
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::Norm2Bias).toTensor(),
      "unpack_norm2_bias_readback"));
  unpacked.emplace_back(unpack_sources_.get(Unpacked::Norm2Eps).toDouble());
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::Fc1Weight).toTensor(),
      "unpack_fc1_weight_readback"));
  append_optional_cpu_snapshot_for_unpack(
      unpacked,
      unpack_sources_,
      Unpacked::Fc1Bias,
      "unpack_fc1_bias_readback");
  unpacked.emplace_back(cpu_snapshot_for_unpack(
      unpack_sources_.get(Unpacked::Fc2Weight).toTensor(),
      "unpack_fc2_weight_readback"));
  append_optional_cpu_snapshot_for_unpack(
      unpacked,
      unpack_sources_,
      Unpacked::Fc2Bias,
      "unpack_fc2_bias_readback");
  append_optional_cpu_snapshot_for_unpack(
      unpacked,
      unpack_sources_,
      Unpacked::Ls2Gamma,
      "unpack_ls2_gamma_readback");
...
std::vector<Tensor> run_vision_backbone_stack_context_impl(
  ...
  const char* const segmented_canary_mode =
      stack_owned_command_buffer_segmented_canary_mode();
  ...
  const bool segmented_dispatch_prefix_canary_requested =
      stack_owned_command_buffer_segmented_dispatch_prefix_canary_enabled();
  const bool segmented_tail_context_canary_requested =
      segmented_prefix_canary_requested || segmented_dispatch_prefix_canary_requested;
  ...
  if (!segmented_canary_requested) {
  ...
  } else if (segmented_dispatch_prefix_canary_requested) {
    segmented_stack_owned_command_buffer_plan =
        stack_owned_command_buffer_dispatch_budget_plan(
            stack_shape_plan,
            context->blocks().size());
  } else {
    segmented_stack_owned_command_buffer_plan =
    ...
    if (selected_segment_dispatch_budget_exceeded(
            stack_shape_plan,
            segmented_stack_owned_command_buffer_plan)) {
      segmented_stack_owned_command_buffer_plan.selected_segment_count = 0u;
      segmented_stack_owned_command_buffer_plan.coverage = "none";
      segmented_stack_owned_command_buffer_plan.status =
          "segment_plan_rejected_behavior_neutral";
      segmented_stack_owned_command_buffer_plan.fail_reason =
          "segment_planned_dispatch_limit_exceeded";
    }
  }
  ...
  if (api::stack_region_recording_domain_observation_enabled()) {
    ...
    api::note_stack_region_segment_plan(
        ...
        segmented_stack_owned_command_buffer_plan.selected_segment_count,
        segmented_stack_owned_command_buffer_plan.coverage,
  ...
  for (size_t block_idx = 0u; block_idx < context->blocks().size(); ++block_idx) {
    if (segmented_stack_owned_command_buffer_canary && !planned_recording_scope) {
      const bool selected_external_segment =
          segmented_stack_owned_command_buffer_segment_index <
          segmented_stack_owned_command_buffer_plan.selected_segment_count;
      const bool context_tail_segment =
          segmented_tail_context_canary_requested && !selected_external_segment &&
          segmented_stack_owned_command_buffer_segment_index <
              segmented_stack_owned_command_buffer_plan.segment_ends.size();
      if (selected_external_segment || context_tail_segment) {
        const size_t segment_end =
        ...
        planned_recording_scope =
            std::make_unique<VulkanStackCommandRecordingScope>(
                *api::context(),
                selected_external_segment);
        if (selected_external_segment) {
