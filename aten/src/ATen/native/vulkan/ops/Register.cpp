...
int register_vulkan_graph_conv2d_relu_plan() {
  static auto register_vulkan_graph_conv2d_relu_plan =
      torch::selective_class_<utils::GraphConv2dReluPlan>(
          "vulkan", TORCH_SELECTIVE_CLASS("GraphConv2dReluPlan"));
  return 0;
}
...
class GraphConv2dReluInvocation final {
 private:
  utils::GraphConv2dReluPlan& plan_;

 public:
  explicit GraphConv2dReluInvocation(utils::GraphConv2dReluPlan& plan)
      : plan_(plan) {
    TORCH_CHECK(
        plan_.try_begin_invocation(),
        "StaticConv2dReluRegion.v1 rejects concurrent invocation");
  }
...
vulkan_graph_region_conv2d_scratch_descriptor(
  ...
  if (
      ...
      !conv_context || conv_context->quantized() ||
      conv_context->transposed() ||
      conv_context->packed_weight().execution_layout() !=
          api::ExecutionLayout::BUFFER_DIRECT ||
      !std::all_of(
          conv_context->output_padding().begin(),
          conv_context->output_padding().end(),
...
utils::GraphConv2dReluPlan::GraphConv2dReluPlan(
  ...
  TORCH_CHECK(
      conv_context_,
      "StaticConv2dReluRegion.v1 requires a static Conv2dPackedContext");
}

const utils::StaticConv2dReluPlanSchema&
utils::GraphConv2dReluPlan::schema() const {
  return schema_;
}

const c10::intrusive_ptr<Conv2dPackedContext>&
utils::GraphConv2dReluPlan::conv_context() const {
  return conv_context_;
}

bool utils::GraphConv2dReluPlan::try_begin_invocation() {
  return !invocation_active_.test_and_set(std::memory_order_acquire);
}

void utils::GraphConv2dReluPlan::end_invocation() {
  invocation_active_.clear(std::memory_order_release);
}

c10::intrusive_ptr<utils::GraphConv2dReluPlan>
utils::create_graph_conv2d_relu_plan(
    const c10::intrusive_ptr<Conv2dPackedContext>& conv_context) {
  return c10::make_intrusive<utils::GraphConv2dReluPlan>(conv_context);
}

Tensor utils::run_graph_conv2d_relu_plan(
    const Tensor& input,
    const c10::intrusive_ptr<utils::GraphConv2dReluPlan>& plan) {
  TORCH_CHECK(plan, "StaticConv2dReluRegion.v1 requires a plan");
  TORCH_CHECK(
      input.is_vulkan(),
      "StaticConv2dReluRegion.v1 requires a Vulkan input tensor");
  TORCH_CHECK(
      plan->schema().instruction_count == 1u &&
          plan->schema().direct_transition_only &&
          plan->schema().replay_state_empty,
...
std::optional<Tensor> run_vulkan_graph_region_plan_impl(
    ...
    Tensor* const output_target) {
  TORCH_CHECK(plan, "VulkanGraphRegionPlan.v1 requires a plan");
  TORCH_CHECK(
      input.is_vulkan(),
      "VulkanGraphRegionPlan.v1 requires a Vulkan input tensor");
  ...
  try {
    ...
    if (expected_descriptor) {
      scratch_slot = plan->find_reusable_scratch_slot(
          *expected_descriptor, context);
      if (scratch_slot >= 0) {
      ...
      if (!reused_scratch) {
        scratch_slot = plan->find_capture_scratch_slot(context);
        if (scratch_slot < 0) {
          api::vulkan_graph_program_invocation_counters()
              .scratch_transient_overflow_count.fetch_add(
                  1u, std::memory_order_relaxed);
        } else {
          Tensor scratch;
          try {
            scratch = utils::create_buffer_tensor(
                expected_descriptor->sizes, kFloat, /*persistent=*/true);
    ...
    Tensor output = output_target
        ? run_conv2d_context_out(intermediate, second_context, *output_target)
        : run_conv2d_context(intermediate, second_context);
    if (
...
Tensor utils::run_vulkan_graph_region_plan(
    const Tensor& input,
    const c10::intrusive_ptr<VulkanGraphRegionPlan>& plan) {
  auto output = run_vulkan_graph_region_plan_impl(input, plan, nullptr);
  TORCH_INTERNAL_ASSERT(output);
  return std::move(*output);
}

std::optional<Tensor> utils::try_run_vulkan_graph_region_plan_out(
    const Tensor& input,
    const c10::intrusive_ptr<VulkanGraphRegionPlan>& plan,
    Tensor& output) {
  return run_vulkan_graph_region_plan_impl(input, plan, &output);
}
...
std::vector<int64_t> submit_origin_counters_runtime() {
  ...
  return {
      ...
      static_cast<int64_t>(
          counters.pending_command_flush.load(std::memory_order_relaxed)),
...
std::vector<std::string> stack_temp_lifetime_safety_snapshot_runtime() {
...
std::vector<int64_t> stack_retire_drain_blocker_counters_runtime() {
  return api::stack_retire_drain_blocker_counters_snapshot();
}

std::vector<std::string> stack_retire_drain_blocker_snapshot_runtime() {
...
TORCH_LIBRARY(vulkan_prepack, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      ...
      "__torch__.torch.classes.vulkan.Conv2dPackedContext context) "
      "-> __torch__.torch.classes.vulkan.GraphConv2dReluPlan"));
  m.def(TORCH_SELECTIVE_SCHEMA(
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "vulkan_prepack::stack_temp_lifetime_safety_snapshot() -> str[]"));
  m.def(TORCH_SELECTIVE_SCHEMA(
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "vulkan_prepack::stack_retire_drain_blocker_counters() -> int[]"));
  m.def(TORCH_SELECTIVE_SCHEMA(
      "vulkan_prepack::stack_retire_drain_blocker_snapshot() -> str[]"));
  m.def(TORCH_SELECTIVE_SCHEMA(
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "vulkan_prepack::stack_attention_counters() -> int[]"));
  m.def(TORCH_SELECTIVE_SCHEMA(
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "vulkan_prepack::stack_program_owned_temp_slot_identity() -> str[]"));
  m.def(TORCH_SELECTIVE_SCHEMA(
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      ...
      "-> __torch__.torch.classes.vulkan.VulkanGraphRegionPlan"));
  m.def(TORCH_SELECTIVE_SCHEMA(
      "vulkan_prepack::run_vulkan_graph_region_plan(Tensor input, "
...
TORCH_LIBRARY_IMPL(vulkan_prepack, CatchAll, m) {
  ...
  m.impl(
      TORCH_SELECTIVE_NAME(
          "vulkan_prepack::stack_temp_lifetime_safety_snapshot"),
      TORCH_FN(stack_temp_lifetime_safety_snapshot_runtime));
  m.impl(
      TORCH_SELECTIVE_NAME(
          "vulkan_prepack::reset_stack_temp_lifetime_safety_snapshot"),
  ...
  m.impl(
      TORCH_SELECTIVE_NAME(
          "vulkan_prepack::stack_retire_drain_blocker_counters"),
      TORCH_FN(stack_retire_drain_blocker_counters_runtime));
  m.impl(
      TORCH_SELECTIVE_NAME("vulkan_prepack::stack_retire_drain_blocker_snapshot"),
      TORCH_FN(stack_retire_drain_blocker_snapshot_runtime));
  m.impl(
      TORCH_SELECTIVE_NAME(
          "vulkan_prepack::reset_stack_retire_drain_blocker_counters"),
  ...
  m.impl(
      TORCH_SELECTIVE_NAME("vulkan_prepack::clone_requirement_snapshot"),
      TORCH_FN(clone_requirement_snapshot));
  m.impl(
  ...
  m.impl(
      TORCH_SELECTIVE_NAME("vulkan_prepack::stack_attention_counters"),
      TORCH_FN(stack_attention_counters_snapshot));
  m.impl(
  ...
  m.impl(
      TORCH_SELECTIVE_NAME(
          "vulkan_prepack::stack_program_owned_temp_slot_identity"),
      TORCH_FN(stack_program_owned_temp_slot_identity_snapshot));
  m.impl(
      TORCH_SELECTIVE_NAME(
...
TORCH_LIBRARY_IMPL(vulkan_prepack, Vulkan, m) {
  ...
  m.impl(
      TORCH_SELECTIVE_NAME(
          "vulkan_prepack::run_vision_stack_captures_decoder_preprocess_bridge"),
      TORCH_FN(run_vision_stack_captures_decoder_preprocess_bridge));
  m.impl(
      TORCH_SELECTIVE_NAME(
          "vulkan_prepack::run_depth_anything_v2_compiled_session_bridge"),
