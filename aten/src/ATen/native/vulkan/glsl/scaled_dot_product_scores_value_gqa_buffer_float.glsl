...
layout(set = 0, binding = 8) uniform restrict Block {
  ivec4 sizes;      // batch, query_heads, target_len, source_len
  ivec4 tiled_info; // head_dim, value_dim, repeat_factor, is_causal
...
void main() {
  ...
  const int batch_query_head = int(gl_WorkGroupID.z);
  const int batch = batch_query_head / uBlock.sizes.y;
  const int query_head = batch_query_head - batch * uBlock.sizes.y;
  const int key_value_head = query_head / uBlock.tiled_info.z;

  if (
      batch >= uBlock.sizes.x || query_head >= uBlock.sizes.y ||
      query_row >= uBlock.sizes.z) {
    return;
  }
  ...
  for (int dx = lane; dx < uBlock.tiled_info.y; dx += LOCAL_SIZE_X) {
  ...
  int query_count = 0;
  for (int k = lane; k < uBlock.tiled_info.x; k += LOCAL_SIZE_X) {
    if (query_count >= MAX_QUERY_VALUES_PER_THREAD) {
      break;
    }
    const uint query_idx = buffer_idx(
        uvec4(uint(k), uint(query_row), uint(query_head), uint(batch)),
  ...
  for (int source_index = 0; source_index < uBlock.sizes.w; ++source_index) {
    const bool masked =
        uBlock.tiled_info.w != 0 && source_index > query_row;
    ...
    if (lane == 0) {
      const float score =
          masked ? NEG_INF : sScorePartials[0] * uBlock.params.x;
