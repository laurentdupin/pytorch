...
void main() {
  ...
  const uint in_base = uInMeta.info.w +
      out_x * uInMeta.physical_strides.x +
      out_y * uInMeta.physical_strides.y +
      batch * uInMeta.physical_strides.w;
  const uint weight_out_base =
      uWeightMeta.info.w + out_channel * uWeightMeta.physical_strides.w;

  float acc = 0.0;
  for (uint in_channel = 0u; in_channel < in_channels; ++in_channel) {
    const uint input_idx = in_base + in_channel * uInMeta.physical_strides.z;
    const uint weight_idx =
        weight_out_base + in_channel * uWeightMeta.physical_strides.z;
