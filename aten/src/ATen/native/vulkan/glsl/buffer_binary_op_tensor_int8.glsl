...
layout(set = 0, binding = 4) buffer restrict readonly OtherBuffer {
  DATA_T data[];
}
uOther;

layout(set = 0, binding = 5) uniform restrict OtherMeta {
  uvec4 logical_sizes;
  uvec4 logical_strides;
  uvec4 physical_strides;
  uvec4 info;
...
layout(set = 0, binding = 6) uniform restrict TensorBlock {
  int alpha;
}
uArgs;
...
void main() {
  ...
  if (all(lessThan(input_coord, uInMeta.logical_sizes))) {
    ...
    if (read_idx < in_buf_length) {
      input_value = int(uInput.data[read_idx]);
    }
  }
  ...
  if (all(lessThan(other_coord, uOtherMeta.logical_sizes))) {
    ...
    if (read_idx < other_buf_length) {
      other_value = int(uOther.data[read_idx]);
    }
  }
  ...
  if (actual_write_idx < out_buf_length) {
    uOutput.data[actual_write_idx] = DATA_T(OP(input_value, other_value, uArgs.alpha));
  }
}
