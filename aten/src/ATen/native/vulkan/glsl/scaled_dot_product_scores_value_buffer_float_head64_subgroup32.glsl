...
void main() {
  ...
  for (int source_index = 0; source_index < uBlock.sizes.z; ++source_index) {
    ...
    const float subgroup_sum = subgroupAdd(query_value * key_value);
    if (gl_SubgroupInvocationID == 0u) {
      sSubgroupSums[gl_SubgroupID] = subgroup_sum;
    }
    barrier();

    if (lane == 0u) {
      sScore = sSubgroupSums[0] + sSubgroupSums[1];
    }
    barrier();

    const float score = sScore * uBlock.params.x;
