...
void main() {
  ...
  const int channel = int(out_coord.x);
  const int token = int(out_coord.y);
  const int batch = int(out_coord.z);
  const int spatial_width = uBlock.info.x;
  const int spatial_height = uBlock.info.y;
  const int channels = uBlock.info.z;
  const int h = token / spatial_width;
  const int w = token - h * spatial_width;

  const ivec4 input_coord = ivec4(batch, channel, h, w);
  const ivec4 input_sizes =
      ivec4(uBlock.info.w, channels, spatial_height, spatial_width);
