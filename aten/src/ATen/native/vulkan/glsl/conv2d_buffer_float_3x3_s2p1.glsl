...
void main() {
  ...
  const int base_x = int(out_x) * 2 - 1;
  const int base_y = int(out_y) * 2 - 1;
  ...
  for (uint in_channel = 0u; in_channel < in_channels; ++in_channel) {
    ...
    if (base_x >= 0 && base_y >= 0 && uint(base_x + 2) < in_width &&
        uint(base_y + 2) < in_height) {
      const uint row0 = input_channel_base +
          uint(base_y) * uInMeta.physical_strides.y +
          uint(base_x) * uInMeta.physical_strides.x;
      ...
      if (base_y >= 0) {
        ...
        if (base_x + 1 >= 0 && uint(base_x + 1) < in_width) {
          acc += uInput.data[row0 + uint(base_x + 1) * uInMeta.physical_strides.x] *
              uWeight.data[w01];
        }
        if (base_x + 2 >= 0 && uint(base_x + 2) < in_width) {
          acc += uInput.data[row0 + uint(base_x + 2) * uInMeta.physical_strides.x] *
              uWeight.data[w02];
        }
      }
      if (base_y + 1 >= 0 && uint(base_y + 1) < in_height) {
        const uint row1 = input_channel_base +
            uint(base_y + 1) * uInMeta.physical_strides.y;
        if (base_x >= 0) {
          acc += uInput.data[row1 + uint(base_x) * uInMeta.physical_strides.x] *
              uWeight.data[w10];
        }
        if (base_x + 1 >= 0 && uint(base_x + 1) < in_width) {
          acc += uInput.data[row1 + uint(base_x + 1) * uInMeta.physical_strides.x] *
              uWeight.data[w11];
        }
        if (base_x + 2 >= 0 && uint(base_x + 2) < in_width) {
          acc += uInput.data[row1 + uint(base_x + 2) * uInMeta.physical_strides.x] *
              uWeight.data[w12];
        }
      }
      if (base_y + 2 >= 0 && uint(base_y + 2) < in_height) {
        const uint row2 = input_channel_base +
            uint(base_y + 2) * uInMeta.physical_strides.y;
        if (base_x >= 0) {
          acc += uInput.data[row2 + uint(base_x) * uInMeta.physical_strides.x] *
              uWeight.data[w20];
        }
        if (base_x + 1 >= 0 && uint(base_x + 1) < in_width) {
          acc += uInput.data[row2 + uint(base_x + 1) * uInMeta.physical_strides.x] *
              uWeight.data[w21];
        }
        if (base_x + 2 >= 0 && uint(base_x + 2) < in_width) {
          acc += uInput.data[row2 + uint(base_x + 2) * uInMeta.physical_strides.x] *
              uWeight.data[w22];
        }
      }
    }
