  ...
  if (all(lessThan(pos, uBlock.shader_extents_and_step.xyz))) {
    ...
    for (int j = 0; j < uBlock.shader_extents_and_step.w; ++j) {
      ...
      for (int k = 0; k < FOUR; ++k) {
        const int input_row = (FOUR * pos.y) + k;
        vec4 row_values = vec4(0.0);
        if (input_row < uBlock.input_sizes.z) {
          for (int lane = 0; lane < FOUR; ++lane) {
            const int input_col = (FOUR * j) + lane;
            if (input_col < uBlock.input_sizes.w) {
              row_values[lane] = texelFetch(uM1, ivec3(input_col, input_row, 0), 0).x;
            }
          }
        }
        uM1_partial_rows[k] = row_values;
      }
    ...
    for (int idx_c = 0; idx_c < FOUR; ++idx_c) {
      ...
      if (output_row >= uBlock.shader_extents_and_step.y) {
        continue;
      }
      const int bias_row = (uBlock.bias_extents.y <= 1) ? 0 : output_row;

      for (int idx_r = 0; idx_r < FOUR; ++idx_r) {
        const int output_col = idx_r + FOUR * pos.x;
        if (output_col >= uBlock.shader_extents_and_step.x) {
          continue;
        }
        ...
        imageStore(
            uOutput,
            ivec3(output_col, output_row, pos.z),
            vec4(
