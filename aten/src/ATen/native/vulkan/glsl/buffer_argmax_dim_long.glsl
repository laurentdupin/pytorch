...
void main() {
  ...
  if (write_idx >= out_numel || reduce_size == 0u) {
    return;
  }
  ...
  const uint actual_write_idx =
      ...
      out_storage_offset;
  const uint word_idx = actual_write_idx * 2u;
  const uint out_word_length = out_buf_length * 2u;
  if (word_idx + 1u < out_word_length) {
    uOutput.data[word_idx] = int(best_index);
    uOutput.data[word_idx + 1u] = 0;
  }
}
