...
layout(set = 0, binding = 8) buffer restrict readonly ScaleBuffer {
  float data[];
}
uScale;

layout(set = 0, binding = 9) uniform restrict ScaleMeta {
  uvec4 logical_sizes;
  uvec4 logical_strides;
  uvec4 physical_strides;
  uvec4 info;
...
layout(set = 0, binding = 10) buffer restrict readonly WeightBuffer {
  float data[];
}
uWeight;

layout(set = 0, binding = 11) uniform restrict WeightMeta {
  uvec4 logical_sizes;
  uvec4 logical_strides;
  uvec4 physical_strides;
  uvec4 info;
...
layout(set = 0, binding = 12) buffer restrict readonly BiasBuffer {
  float data[];
}
uBias;

layout(set = 0, binding = 13) uniform restrict BiasMeta {
  uvec4 logical_sizes;
  uvec4 logical_strides;
  uvec4 physical_strides;
  uvec4 info;
...
layout(set = 0, binding = 14) uniform restrict Params {
  vec4 data;
}
uParams;
...
void main() {
  ...
  const uint normalized_size = 384u;
  ...
  float partial_sumsq = 0.0;
  for (uint x = lane; x < normalized_size; x += 256u) {
    ...
    const uint scale_idx =
        coord_to_idx(uvec4(x, 0u, 0u, 0u), uScaleMeta.physical_strides) +
    ...
    if (
      ...
      partial_sum += value;
      partial_sumsq += value * value;
    }
  }

  s_sum[lane] = partial_sum;
  s_sumsq[lane] = partial_sumsq;
  barrier();

  for (uint offset = 128u; offset > 0u; offset >>= 1u) {
    if (lane < offset) {
      s_sum[lane] += s_sum[lane + offset];
      s_sumsq[lane] += s_sumsq[lane + offset];
    }
    barrier();
  }

  const float mean = s_sum[0] / float(normalized_size);
  const float variance =
      max(s_sumsq[0] / float(normalized_size) - mean * mean, 0.0);
