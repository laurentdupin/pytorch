...
void main() {
  ...
  const uint group_idx = gl_GlobalInvocationID.x;
  const uint chunk_size = uint(uBlock.info.x);
  const uint in_numel = uInMeta.info.y;
  const uint in_buf_length = uInMeta.info.z;
  const uint in_storage_offset = uInMeta.info.w;

  const uint start_idx = group_idx * chunk_size;
  if (start_idx >= in_numel) {
    return;
  }

  const uint end_idx = min(start_idx + chunk_size, in_numel);
  float acc = INIT;
  for (uint logical_idx = start_idx; logical_idx < end_idx; ++logical_idx) {
    const uvec4 coord =
  ...
  uOutput.data[group_idx] = FINALIZE(acc, float(end_idx - start_idx));
}
