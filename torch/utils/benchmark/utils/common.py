...
class Measurement:
    ...
    def significant_figures(self) -> int:
        ...
        std = float(torch.tensor(interquartile_points).std(unbiased=False).item())
        sqrt_n = float(torch.tensor(len(interquartile_points)).sqrt().item())
    ...
    def _lazy_init(self) -> None:
        if self.raw_times and not self._sorted_times:
            ...
            self._median = float(_sorted_times.quantile(.5).item())
            self._mean = float(_sorted_times.mean().item())
            self._p25 = float(_sorted_times.quantile(.25).item())
            self._p75 = float(_sorted_times.quantile(.75).item())
