...
class InvokeSubgraphCache(HopSubgraphCache):
    def __init__(self) -> None:
        ...
        # fn_id → {hash_key → cache_entry}. Used by user-provided
        # reuse_hash_fn for O(1) subgraph reuse lookup.
        self.subgraph_reuse_key_cache: dict[
            int, dict[int, InvokeSubgraphReuseEntry]
        ] = defaultdict(dict)
    ...
    def find_reuse_entry_by_key(
        self, fn_id: int, hash_key: int
    ) -> InvokeSubgraphReuseEntry | None:
        return self.subgraph_reuse_key_cache.get(fn_id, {}).get(hash_key)
    ...
    def add_reuse_entry_by_key(
        ...
        key_cache = self.subgraph_reuse_key_cache[fn_id]
        if len(key_cache) >= max_reuse_entries and hash_key not in key_cache:
            raise RuntimeError(
                f"invoke_subgraph: exceeded maximum reuse entries "
                f"({max_reuse_entries}) for function id {fn_id} (hash-key path). "
