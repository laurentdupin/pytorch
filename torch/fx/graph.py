...
class _CodeGenContext:
    ...
    blue: Callable[[str], str] = _identity
    dim: Callable[[str], str] = _identity
    red: Callable[[str], str] = _identity
    dim_green: Callable[[str], str] = _identity
    dim_blue: Callable[[str], str] = _identity
...
class CodeGen:
    ...
    def _gen_python_code(
        ...
        *,
        ...
        if colored:
            ctx.red = _color_fns["red"]
            ctx.dim_green = _color_fns["dim_green"]
            ctx.dim = _color_fns["dim"]
            ctx.dim_blue = _color_fns["dim_blue"]
            ctx.blue = _color_fns["blue"]
        ...
        def append_stacktrace_summary(node: Node):
            ...
            if node.op not in {"placeholder", "output"}:
                ...
                summary_str = f"\n{ctx.dim(f'{additional_meta_str}#{annotation_str}{maybe_recompute_info} {stack_trace_str}')}\n"
        ...
        for i, node in enumerate(nodes):
            # NOTE: _codegen_emit_node does not emit a string with newline.
            # It depends on _codegen_delete_unused_values to append one.
            if verbose:
            ...
            if verbose:
                ...
                def _tensor_annotation(t: torch.Tensor) -> str:
                    ...
                    return (
                        f"{ctx.red(dtype_abbrs[t.dtype])}"
                        f"{ctx.blue(stringify_shape(t.shape))}"
                        f"{ctx.dim_blue(stride)}"
                        f"{ctx.dim_green(device)}"
                    )
                ...
                if isinstance(meta_val, torch.Tensor) and meta_val.layout not in (
                    ...
                    if is_plain:
                        ...
                        maybe_type_annotation = (
                            f': "{cls}({core}, {ctx.dim_green(dtensor_meta)})"'
                        )
                    else:
                        cls = meta_val.__class__.__name__
            ...
            self._codegen_delete_unused_values(ctx, node, [body])
            if do_record:
...
{code}"""
    ...
    # ------------------------------------------------------------------
    # Helper methods for codegen logic.
    #
    # These operate on an explicit _CodeGenContext.  Both the base
    # _gen_python_code() and subclasses (e.g. ProfilerCodeGen) call
    # these methods, avoiding duplicated logic.
    # ------------------------------------------------------------------
    ...
    def _codegen_delete_unused_values(
        ...
        if len(nodes_to_delete):
            ...
            cleanup = f";  {ctx.dim(to_delete_str)}\n"
            for body in bodies:
                body.append(cleanup)
        else:
            for body in bodies:
