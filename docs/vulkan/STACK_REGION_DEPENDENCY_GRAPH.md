...
`diagnostic_payload_publish_mode=deferred_after_context_unlock`, and the
before/after retained-state payload capture flags with
`retained_state_live_log_reread_count=0` and
...
payloads after the context lock is released instead of rereading the live
context at publication time. The executor can later move out of the stack-exit
